...
extern volatile U32 g_secondaryResetFlag;
...
OS_SEC_ALW_INLINE INLINE U32 OsTaskCreateChkAndGetTcb(struct TagTskCb **taskCb, bool isSmpIdle)
{
    if (isSmpIdle) {
        *taskCb = &g_tskCbArray[g_tskMaxNum + THIS_CORE()];
    } else {
        OsTskRecycle();
...
OS_SEC_ALW_INLINE INLINE bool OsCheckStackAddrOverflow(uintptr_t base, size_t size)
{
    return OsCheckAddrOffsetOverflow(base, size);
}
#else
OS_SEC_ALW_INLINE INLINE bool OsCheckStackAddrOverflow(uintptr_t base, size_t size)
{
    return ((U64)base + (U64)size) > OS_MAX_U32;
}
#endif
