...
static U32 OsDynModuleRelocateAbs64(Elf_Addr relocAddr, struct OsDynModuleRelocInfo relocInfo)
{
    Elf_Sword addend = 0;
    if (relocInfo.shType == SHT_RELA) {
        addend = ((Elf_Rela *)(relocInfo.reloc))->r_addend;
    } else {
        addend = *(Elf_Addr *)relocAddr;
    }
    *(Elf_Addr *)relocAddr = relocInfo.symAddr + addend;
    return OS_MODULE_OK;
}
...
const struct OsDynModuleRelocateMap g_relocateMap[] = {
    { R_AARCH64_ABS64, OsDynModuleRelocateAbs64 },
    { R_AARCH64_GLOB_DAT, OsDynModuleRelocateGlobDat },
    { R_AARCH64_JUMP_SLOT, OsDynModuleRelocateJumpSlot },
    { R_AARCH64_RELATIVE, OsDynModuleRelocateRelative },
};
