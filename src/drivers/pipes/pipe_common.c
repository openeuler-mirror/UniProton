...
int pipecommon_open(FAR struct file *filep)
{
   ...
   * instance.
   */
  int fg = filep->f_oflags & O_RWMASK;
  if ((fg == O_WRONLY || fg == O_RDWR))
    {
      dev->d_nwriters++;
  ...
  while ((filep->f_oflags & O_NONBLOCK) == 0 &&     /* Blocking */
         (filep->f_oflags & O_RWMASK) == O_WRONLY &&  /* Write-only */
...
int pipecommon_poll(FAR struct file *filep, FAR struct __pollfd *fds,
                    bool setup)
{
  FAR struct inode      *inode = filep->f_inode;
  FAR struct pipe_dev_s *dev   = inode->i_private;
  ...
  if (setup)
    {
      ...
      if ((fg == O_RDONLY || fg == O_RDWR) && (nbytes > dev->d_pollinthrd))
        {
          eventset |= POLLIN;
        }
