...
typedef CODE int (*xcpt_t)(FAR void *arg);
#endif /* __ASSEMBLY__ */

/* Adapte irqstate_t for OsIntLock and OsIntRestore */
typedef uintptr_t irqstate_t;
 ...
 *
 *     This function is equivalent to OsIntLock().
 *
 * Input Parameters:
 *   None
 *
...
#else
#  define enter_critical_section() OsIntLock()
#endif
 ...
 *   If thread-specific IRQ counter is not enabled:
 *
 *     This function is equivalent to OsIntRestore().
 *
 * Input Parameters:
...
#else
#  define leave_critical_section(f) OsIntRestore(f)
#endif
