...
/* Values for the struct TagTskCb flags bits */
...
#  define TCB_PID_OFF                offsetof(struct TagTskCb, pid)
#  define TCB_STATE_OFF              offsetof(struct TagTskCb, task_state)
#  define TCB_PRI_OFF                offsetof(struct TagTskCb, sched_priority)
#if CONFIG_TASK_NAME_SIZE > 0
#  define TCB_NAME_OFF               offsetof(struct TagTskCb, name)
#else
#  define TCB_NAME_OFF               0
#endif
#  define TCB_REGS_OFF               offsetof(struct TagTskCb, xcp.regs)
 ...
 * The range of values is assumed to fit into a uint8_t in struct TagTskCb.
 */
...
{
  FAR struct TagTskCb *tcb;            /* The parent TCB structure             */
...
struct task_tcb_s
{
  ...
  struct TagTskCb cmn;                      /* Common TCB fields               */
...
typedef CODE void (*nxsched_foreach_t)(FAR struct TagTskCb *tcb, FAR void *arg);
...
FAR struct TagTskCb *nxsched_get_tcb(pid_t pid);
...
int nxsched_release_tcb(FAR struct TagTskCb *tcb, uint8_t ttype);
...
void nxtask_activate(FAR struct TagTskCb *tcb);
...
size_t group_argvstr(FAR struct TagTskCb *tcb, FAR char *args, size_t size);
...
void nxsched_resume_scheduler(FAR struct TagTskCb *tcb);
#else
...
void nxsched_suspend_scheduler(FAR struct TagTskCb *tcb);
#else
...
void nxsched_get_stateinfo(FAR struct TagTskCb *tcb, FAR char *state,
                           size_t length);
