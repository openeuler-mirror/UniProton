...
int sys_mknod(FAR const char *path, mode_t mode, dev_t dev)
{
  int ret = -1;
  ...
  switch (mode & S_IFMT)
    {
      ...
      case S_IFIFO:
        ret = sys_mkfifo(path, mode & ~S_IFMT);
        break;
#endif

      case S_IFDIR:
        ret = sys_mkdir(path, mode & ~S_IFMT);
        break;

      case S_IFREG:
        ret = sys_open(path, O_CREAT|O_WRONLY|O_TRUNC, mode & ~S_IFMT);
        if (ret >= 0)
          {
            ret = sys_close(ret);
          }
...
int sys_mknodat(int dirfd, FAR const char *path, mode_t mode, dev_t dev)
{
  ...
  return sys_mknod(fullpath, mode, dev);
}
