#ifndef PRT_BESTFIT_SLAB_PRI_H
#define PRT_BESTFIT_SLAB_PRI_H

#include "prt_bestfit_slab.h"
...
typedef struct tagPrtBestfitSlabStatus {
    UINT32 totalSize;
    UINT32 usedSize;
    UINT32 freeSize;
    UINT32 allocCount;
    UINT32 freeCount;
} PrtBestfitSlabStatus;

typedef struct tagPrtBestfitSlabBlockNode {
    UINT16 magic;
    UINT8 blkSz;
    UINT8 recordId;
    UINT32 reserved;
...
typedef struct tagPrtBestfitSlabAllocator {
    UINT32 itemSz;
    UINT8 *dataChunks;
...
typedef struct tagPrtBestfitSlabMemAllocator {
    struct tagPrtBestfitSlabMemAllocator *next;
    OsSlabAllocator *slabAlloc;
} OsSlabMemAllocator;
#endif

typedef struct tagPrtBestfitSlabMem {
    UINT32 blkSz;
    UINT32 blkCnt;
    UINT32 blkUsedCnt;
#ifdef PRT_BESTFIT_CFG_KERNEL_MEM_SLAB_AUTO_EXPANSION_MODE
...
struct PrtBestfitSlabControlHeader {
#ifdef PRT_BESTFIT_CFG_KERNEL_MEM_SLAB_AUTO_EXPANSION_MODE
...
extern VOID *OsBestfitHeapAlloc(VOID *pool, UINT32 size);
extern UINT32 OsBestfitHeapFree(VOID *pool, VOID *mem);
...
extern UINT32 OsSlabStatisticsGet(const VOID *pool, PrtBestfitSlabStatus *status);
...
#endif /* PRT_BESTFIT_CFG_KERNEL_MEM_SLAB_EXTENTION */

#endif /* PRT_BESTFIT_SLAB_PRI_H */
