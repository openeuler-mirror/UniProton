...
elif [ "${CPU_TYPE}" = "raspi4" ] || [ "${CPU_TYPE}" = "hi3093" ] || [ "${CPU_TYPE}" = "rk3568_jailhouse" ];
...
pushd "$CK_LIB_PATH"
##为什么不加这一行要报错
if [ "${CPU_TYPE}" = "m4" ] || [ "${CPU_TYPE}" = "raspi4" ] || [ "${CPU_TYPE}" = "hi3093" ] || [ "${CPU_TYPE}" = "x86_64" ] || [ "${CPU_TYPE}" = "rk3568_jailhouse" ] ; then
...
[ -n "${file}" ] && rm -rf "${file}"
if [ "${CPU_TYPE}" = "m4" ] || [ "${CPU_TYPE}" = "raspi4" ] || [ "${CPU_TYPE}" = "hi3093" ] || [ "${CPU_TYPE}" = "rk3568_jailhouse" ]; then  
...
if [ "${CPU_TYPE}" = "raspi4" ] || [ "${CPU_TYPE}" = "hi3093" ] || [ "${CPU_TYPE}" = "rk3568_jailhouse" ]; then
    for i in $(ls *.o);
    do
        if [ -f "${i}" ]  ; then
