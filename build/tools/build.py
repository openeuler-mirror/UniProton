...
UniProton_home = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, "%s/tools"%UniProton_home)
...
class Compile:
    ...
    def __init__(self, cpu_type: str, make_option="normal", lib_run_type="FPGA", choice="ALL", make_phase="ALL",
        ...
        self.UniProton_packing_path = UniProton_packing_path if make_phase == "CREATE_CMAKE_FILE" else '%s/build/output'%self.home_path
    ...
    def CMake(self):
        ...
        if self.make_phase in ['CREATE_CMAKE_FILE', 'ALL']:
            ...
            if self.compile_option == 'fortify':
                cmd = '%s/cmake %s -DCMAKE_TOOLCHAIN_FILE=%s/build/cmake/tool_chain/uniproton_tool_chain.cmake ' \
