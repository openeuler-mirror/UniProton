...
if [[ "${APP}" == "cxxTest" || "${APP}" == "eigenTest" ]]
then
    cmake -S .. -B $TMP_DIR -DAPP:STRING=$APP -DTOOLCHAIN_PATH:STRING=$TOOLCHAIN_PATH -DCXX_TOOLCHAIN_PATH:STRING=$CXX_TOOLCHAIN_PATH -DCPU_TYPE:SRTING="hi3093" -DCMAKE_TRY_COMPILE_TARGET_TYPE=STATIC_LIBRARY
else
    cmake -S .. -B $TMP_DIR -DAPP:STRING=$APP -DTOOLCHAIN_PATH:STRING=$TOOLCHAIN_PATH -DCPU_TYPE:SRTING="hi3093" -DCMAKE_TRY_COMPILE_TARGET_TYPE=STATIC_LIBRARY
fi
