 ...
 * Description: share memory info head file;
 * Create: 2023/09/06
 */
...
#define SHM_OP_START 0              // 操作 初始状态，等待
#define SHM_OP_READY_TO_READ 1      // 操作 等待读取
#define SHM_OP_READ_END 2           // 操作 读取完毕
#define SHM_OP_DEL 3                // 操作 等待删除
...
struct shm_ipi_func {
    int (* init)(void);
    int (* send_ipi)(int fd, struct cpu_info info);
};
