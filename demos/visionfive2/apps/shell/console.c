...
{
    uart_printf("==========task_bef_exc===================\n");
    uart_printf("task_sp : 0x");
    ...
    uart_printf(ibuf);
    uart_printf("\n");
    uart_printf("(here i don't handle this because there is too many flags)\n");
    uart_printf("(if you want to check, do it yourself)\n");

    uart_printf("taskPrio : 0x");
    ...
    uart_printf("taskName : ");
    uart_printf(task->name);
    uart_printf("\n");

    uart_printf("taskEntryAddr : 0x");
    ...
    uart_printf("taskStackBottom : 0x");
...
{
    uart_printf("=============================exc_dump_start======================\n");
    uart_printf("OsVersion : ");
    uart_printf(excInfo->osVer);
    uart_printf("\n");

    uart_printf("AppVersion : ");
    ...
    uart_printf("Excption IN :");
    switch (excInfo->threadType)
    {
    case EXC_IN_HWI:
        uart_printf("hardware interrupt \n");
        break;
    case EXC_IN_TICK:
        uart_printf("hardware tick interrupt \n");
        break;
    case EXC_IN_SYS:
        uart_printf("system handler\n");
        break;
    case EXC_IN_TASK:
        uart_printf("task\n");
        uart_printf("threadId: ");
        ...
        break;
    default:
        uart_printf("type i don't know\n");
        break;
    }
    uart_printf("ByteOrder :");
    if (excInfo->byteOrder == OS_LITTLE_ENDIAN)
    {
        uart_printf("little endian\n");
    }
    else
    {
        uart_printf("big endian\n");
    }
    ...
    {
    case OS_RV64_VIRT:
        uart_printf("riscv64 qemu virt\n");
        break;
    default:
        uart_printf("unkown type\n");
        break;
    }
...
void console_hwi_test_2()
{
    ...
    {
        uart_printf("add hook entry error\n");
    }
    else
    {
        uart_printf("add hook entry success\n");
    }
    if (PRT_HwiAddExitHook(HwiEntryHook_test_add_tail) != OS_OK)
    {
        uart_printf("add hook tail error\n");
    }
    else
    {
        uart_printf("add hook tail success\n");
    }
    uart_printf(">> ");
}
void console_hwi_test_3()
{
    ...
    {
        uart_printf("del hook entry error\n");
    }
    else
    {
        uart_printf("del hook entry success\n");
    }
    if (PRT_HwiDelExitHook(HwiEntryHook_test_add_tail) != OS_OK)
    {
        uart_printf("del hook tail error\n");
    }
    else
    {
        uart_printf("del hook tail success\n");
    }
    uart_printf(">> ");
}
void console_exc_test()
{
    ...
    {
        uart_printf("add exc hook error\n");
        return;
    }
    uart_printf("add exc hook success\n");
    uart_printf(">> ");
}
...
void console_base_info()
{
    ...
    uart_printf("system version : ");
    uart_printf(OsVersion);
    uart_printf("\n");
    uart_printf("platform : ");
#if (OS_HARDWARE_PLATFORM == OS_RISCV64)
    uart_printf("riscv64\n");
#else
    uart_printf("unkown platform\n");
#endif

    uart_printf("cpuType : ");
#if (OS_CPU_TYPE == OS_RV64_VIRT)
    uart_printf("qemu virt\n");
#else
    uart_printf("unkown board\n");
#endif
...
{
    uart_printf("\e[91msorry,command i don't know please input 'help' to get information\n\e[39m");
    uart_printf(">> ");
}
void console_hwi_test()
{
    ...
    {
        uart_printf("disable hwi uart failed\n");
        return;
    }

    uart_printf("======================disable hwi uart start=============\n");
    uart_printf("from now on, your uart int is disabled for 5s\n");
    PRT_ClkDelayMs(5000);
    uart_printf("======================disable hwi uart end===============\n");
...
void console_help()
{
    uart_printf("\e[92msystem      :  \e[95m show system information \n\e[39m");

    uart_printf("\e[92mhwitest_1   :  \e[95m test hwi uapi enable and disable \n\e[39m");

    uart_printf("\e[92mhelp        :  \e[95m get command help \n\e[39m");

    uart_printf("\e[92mhwitest_2   :  \e[95m test hwi uapi hook add \n\e[39m");

    uart_printf("\e[92mhwitest_3   :  \e[95m test hwi uapi hook del \n\e[39m");

    uart_printf("\e[92mhook_exc    :  \e[95m test exc uapi hook add \n\e[39m");

    uart_printf("\e[92mexc_trig    :  \e[95m test exc uapi trigger excption (maybe you should add exc hook that will let it more good when exc)\n\e[39m");

    uart_printf("\e[92mcreat_thd   :  \e[95m create a thread that will print hello,world once a second but not resume\n\e[39m");

    uart_printf("\e[92mresum_thd   :  \e[95m resume the thread hello,world\n\e[39m");

    uart_printf("\e[92msuspd_thd   :  \e[95m suspend the thread hello,world\n\e[39m");

    uart_printf("\e[92mdele_thd    :  \e[95m delete hello world thread\n\e[39m");

    uart_printf("\e[92mdelay_thd   :  \e[95m delay console  thread for 5s\n\e[39m");

    uart_printf("\e[92mlock_test  :  \e[95m use hello_world thread to lock sched lock for 5s (hello_world thread must be resumed before)\n\e[39m");
    uart_printf(">> ");
}
...
void thread_hello(uintptr_t param1, uintptr_t param2, uintptr_t param3, uintptr_t param4)
{
    while (1)
    {
        ...
        {
            PRT_TaskLock();
            uart_printf("=====on lock for 5 s in hello wordl thread======\n");
            uart_printf("=====high priority thread console thread can't load======\n");
            PRT_ClkDelayMs(5000);
            uart_printf("===unlock ======\n");
            want_lock = 0;
            PRT_TaskUnlock();
...
void console_hello_world()
{
    ...
    if (hello_world == 1)
    {
        uart_printf("do not create repeate one!\n");
        uart_printf(">> ");
        return;
    }
    hello_world = 1;
    ...
    {
        uart_printf("create task fail!\n");
    }
    else
    {
        uart_printf("create task success!\n");
    }
    uart_printf(">> ");
}
...
void console_hello_word_rsm()
{
    if (hello_world != 1)
    {
        uart_printf("do not resume one that is not created!\n");
        uart_printf(">> ");
        return;
    }
    if (PRT_TaskResume(taskPid) != OS_OK)
    {
        uart_printf("resume task fail!\n");
    }
    else
    {
        uart_printf("resume task success!\n");
    }
    uart_printf(">> ");
}
...
void console_hello_word_Suspend()
{
    if (hello_world != 1)
    {
        uart_printf("do not suspend one that is not created!\n");
        uart_printf(">> ");
        return;
    }
    if (PRT_TaskSuspend(taskPid) != OS_OK)
    {
        uart_printf("suspend task fail!\n");
    }
    else
    {
        uart_printf("suspend task success!\n");
    }
    uart_printf(">> ");
}
...
void console_del_hello_world()
{
    if (hello_world == 0)
    {
        uart_printf("hello world don't create\n");
        uart_printf(">> ");
        return;
    }
    if (PRT_TaskDelete(taskPid) != OS_OK)
    {
        uart_printf("del task fail!\n");
    }
    hello_world = 0;
    uart_printf("del task success!\n");
    uart_printf(">> ");
}
...
void console_delay_hello_world()
{
    ...
    {
        uart_printf("delay console task fail!\n");
    }
    uart_printf("delay console task for 5 s success!\n");
    uart_printf(">> ");
}
