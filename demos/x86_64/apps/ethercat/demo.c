...
bool wait_for_slave_respond()
{
    ...
    if (!master_tmp) {
        printf("[TEST] open master fail\n");
        return false;
    }

    periodic_task_init(&pinfo);
    pinfo.period_s = 1;
    printf("[TEST] slave respond wait start: sec:%ld, nsec%ld\n", pinfo.next_period.tv_sec, pinfo.next_period.tv_nsec);

    // wait for slave responding
    ecrt_master_state(master_tmp, &master_state);
    printf("[TEST] start wait, get master state, wait count:%d, num:%u, al_state:0x%x, linkup:%u\n", count,
    ...
    printf("[TEST] finish wait, get master state, wait count:%d, num:%u, al_state:0x%x, linkup:%u\n", count,
           master_state.slaves_responding, master_state.al_states, master_state.link_up);
...
bool wait_for_slave_scan_complete()
{
    ...
    periodic_task_init(&pinfo);
    pinfo.period_s = 1;
    printf("[TEST] slave scan wait start: sec:%ld, nsec%ld\n", pinfo.next_period.tv_sec, pinfo.next_period.tv_nsec);

    // wait for slave scan completion
    ret = ecrt_master(master_tmp, &master_info);
    printf("[TEST] start wait, get master info, scan_busy:%u, slave_count:%u, ret:%d\n", master_info.scan_busy,
           master_info.slave_count, ret);
    ret = ecrt_master_get_slave(master_tmp, 0, &slave_info);
    printf("[TEST] start wait, get slave info, vendor_id:%lx, product_code:%lx, current_on_ebus:%d, al_state:%u, "
           "sdo_count:%u\n",
           slave_info.vendor_id, slave_info.product_code, slave_info.current_on_ebus, slave_info.al_state,
           slave_info.sdo_count);
    while ((master_info.scan_busy || !slave_info.sdo_count || !master_info.slave_count) && count < 10) {
        count++;
        wait_rest_of_period(&pinfo);
        (void)ecrt_master(master_tmp, &master_info);
        (void)ecrt_master_get_slave(master_tmp, 0, &slave_info);
    }
    printf("[TEST] finish wait, get master info, scan_busy:%u, , slave_count:%u, wait count:%d\n",
           master_info.scan_busy, master_info.slave_count, count);
    printf("[TEST] finish wait, get slave info, current_on_ebus:%d, al_state:%u, sdo_count:%u\n",
           slave_info.current_on_ebus, slave_info.al_state, slave_info.sdo_count);

    ecrt_release_master(master_tmp);

    return !master_info.scan_busy && slave_info.sdo_count && master_info.slave_count;
}
