...
export ALL="kp920_lite"

sh ./build_fetch.sh 
sh ./build_static.sh kp920_lite
sh ./build_openamp.sh
...
function build()
{
    ...
    cmake -S .. -B $TMP_DIR -DAPP:STRING=$APP -DTOOLCHAIN_PATH:STRING=$TOOLCHAIN_PATH -DCPU_TYPE:SRTING="kp920_lite" -DCMAKE_TRY_COMPILE_TARGET_TYPE=STATIC_LIBRARY
    pushd $TMP_DIR
    make $APP
    popd
