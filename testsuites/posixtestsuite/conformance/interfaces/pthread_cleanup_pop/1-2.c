...
int pthread_cleanup_pop_1_2_sem1; 			/* Manual semaphore */
int pthread_cleanup_pop_1_2_cleanup_flag;

/* Cleanup handler */
void pthread_cleanup_pop_1_2_a_cleanup_func(void *flag_val)	
{
	pthread_cleanup_pop_1_2_cleanup_flag = (long)flag_val;
	return;
}

/* Function that the thread executes upon its creation */
void *pthread_cleanup_pop_1_2_a_thread_func()
{
	pthread_cleanup_push(pthread_cleanup_pop_1_2_a_cleanup_func, (void*) CLEANUP_CALLED);
	pthread_cleanup_pop(0);
	
	/* Tell main that the thread has called the pop function */	
	pthread_cleanup_pop_1_2_sem1 = INMAIN;

	/* Wait for main to say it's ok to continue (as it checks to make sure that
	 * the cleanup handler was called */
	while (pthread_cleanup_pop_1_2_sem1 == INMAIN)
		sleep(1);
...
int pthread_cleanup_pop_1_2()
{
	pthread_t new_th;

	/* Initializing values */
	pthread_cleanup_pop_1_2_sem1=INTHREAD;
	pthread_cleanup_pop_1_2_cleanup_flag = CLEANUP_NOTCALLED;
	
	/* Create a new thread. */
	if(pthread_create(&new_th, NULL, pthread_cleanup_pop_1_2_a_thread_func, NULL) != 0)
	{	
	...
	while(pthread_cleanup_pop_1_2_sem1==INTHREAD)
		sleep(1);
	
	/* Check to verify that the cleanup handler was called */
	if(pthread_cleanup_pop_1_2_cleanup_flag == CLEANUP_CALLED)
	{
