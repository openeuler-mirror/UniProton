 ...
 * - Send a signal to the child process and have the nanosleep_3_1_handler exit with
 *   success; if the nanosleep_3_1_handler is not called, return 0
...
void nanosleep_3_1_handler(int signo)
{
...
int nanosleep_3_1(int argc, char *argv[])
{
	...
	if ((pid = fork()) == 0) {
		...
		act.sa_handler=nanosleep_3_1_handler;
		act.sa_flags=0;
