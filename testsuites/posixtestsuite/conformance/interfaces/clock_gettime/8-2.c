...
static int clock_gettime_8_2_invalid_tests[NUMINVALIDTESTS] = {
		INT32_MIN, INT32_MAX, 2147483647, -2147483647, -1073743192, 
		1073743192, -1, 17
};

int clock_gettime_8_2(int argc, char *argv[])
{
	struct timespec tp;
	int i;
	int failure = 0;
	...
	for (i = 0; i < NUMINVALIDTESTS; i++) {
		if (clock_gettime(clock_gettime_8_2_invalid_tests[i], &tp) == 0) {
			printf("failure: clock_gettime() using %d succeeded\n",
					clock_gettime_8_2_invalid_tests[i]);
			failure = 1;
		} else {
			if (EINVAL != errno) {
