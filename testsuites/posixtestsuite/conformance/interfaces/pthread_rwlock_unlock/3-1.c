 ...
 * 2.  Main thread write lock 'pthread_rwlock_unlock_3_1_rwlock'
 ...
 *     on write lock too.
 * 7.  Main thread release the 'pthread_rwlock_unlock_3_1_rwlock', make sure the threads got the lock in the order 
...
static pthread_rwlock_t pthread_rwlock_unlock_3_1_rwlock;
static int pthread_rwlock_unlock_3_1_rd_thread_state;
static int pthread_rwlock_unlock_3_1_wr_thread_state_1, pthread_rwlock_unlock_3_1_wr_thread_state_2; 
...
static int pthread_rwlock_unlock_3_1_set_priority(pthread_t pid, unsigned policy, unsigned prio)
{
...
static void* pthread_rwlock_unlock_3_1_fn_rd(void *arg)
{ 
	int rc = 0;
	int priority;
	pthread_rwlock_unlock_3_1_rd_thread_state = ENTERED_THREAD;
	
	priority = (long)arg;
	pthread_rwlock_unlock_3_1_set_priority(pthread_self(), TRD_POLICY, priority);

	printf("reader: attempt read lock\n");
	rc = pthread_rwlock_rdlock(&pthread_rwlock_unlock_3_1_rwlock);
	if(rc != 0)
	{
	...
	pthread_rwlock_unlock_3_1_rd_thread_state = PASSED_LOCK;
	printf("reader: acquired read lock\n");
	
	/* Wait for main to wake us up */
	do{
		sleep(1);
	}while(pthread_rwlock_unlock_3_1_rd_thread_state != EXITING_THREAD);

	printf("reader: unlock read lock\n");
	if(pthread_rwlock_unlock(&pthread_rwlock_unlock_3_1_rwlock) != 0)
	{
...
static void* pthread_rwlock_unlock_3_1_fn_wr_1(void *arg)
{ 
	int rc = 0;
	int priority;
	pthread_rwlock_unlock_3_1_wr_thread_state_1 = ENTERED_THREAD;
	
	priority = (int)(long)arg;
	pthread_rwlock_unlock_3_1_set_priority(pthread_self(), TRD_POLICY, priority);

	printf("writer1: attempt write lock\n");
	rc = pthread_rwlock_wrlock(&pthread_rwlock_unlock_3_1_rwlock);
	if(rc != 0)
	{
	...
	pthread_rwlock_unlock_3_1_wr_thread_state_1 = PASSED_LOCK;
	...
	do{
		sleep(1);
	}while(pthread_rwlock_unlock_3_1_wr_thread_state_1 != EXITING_THREAD);
...
static void* pthread_rwlock_unlock_3_1_fn_wr_2(void *arg)
{ 
	int rc = 0;
	int priority;
	pthread_rwlock_unlock_3_1_wr_thread_state_2 = ENTERED_THREAD;
	...
	pthread_rwlock_unlock_3_1_wr_thread_state_2 = PASSED_LOCK;
	printf("writer2: acquired writer lock\n");

	/* Wait for main to wake us up */
	do{
		sleep(1);
	}while(pthread_rwlock_unlock_3_1_wr_thread_state_2 != EXITING_THREAD);
...
int pthread_rwlock_unlock_3_1()
{
	...
	if(pthread_rwlock_init(&pthread_rwlock_unlock_3_1_rwlock, NULL) != 0)
	{
	...
	if(pthread_rwlock_wrlock(&pthread_rwlock_unlock_3_1_rwlock) != 0)
	{
		printf("Error: main cannot get write lock when no one owns the lock\n");
		return PTS_UNRESOLVED;
	}
	
	/* Now create 3 threads that block on this pthread_rwlock_unlock_3_1_rwlock */
	
	pthread_rwlock_unlock_3_1_wr_thread_state_1 = NOT_CREATED_THREAD;
	priority = sched_get_priority_min(TRD_POLICY) + 2;
	printf("main: create writer1, with priority: %d\n", priority);
	if(pthread_create(&writer1, NULL, pthread_rwlock_unlock_3_1_fn_wr_1, (void*)(long)priority) != 0)
	{
	...
	do{
		sleep(1);
	}while (pthread_rwlock_unlock_3_1_wr_thread_state_1 != 3 && cnt++ < 3); 
	
	if(pthread_rwlock_unlock_3_1_wr_thread_state_1 == 3 )
	{
		printf("writer1 did not block on write lock, when main owns the lock\n");
		return PTS_UNRESOLVED ;
	}
	else if(pthread_rwlock_unlock_3_1_wr_thread_state_1 != 2)
	{
	...
	pthread_rwlock_unlock_3_1_rd_thread_state = 1;
	priority = sched_get_priority_min(TRD_POLICY)+2;
	printf("main: create reader, with priority: %d\n", priority);
	if(pthread_create(&reader, NULL, pthread_rwlock_unlock_3_1_fn_rd, (void*)(long)priority) != 0)
	{
	...
	do{
		sleep(1);
	}while (pthread_rwlock_unlock_3_1_rd_thread_state != 3 && cnt++ < 2); 
	
	if(pthread_rwlock_unlock_3_1_rd_thread_state == 3 )
	{
		printf("Test Fail: reader did not block on read lock\n");
		return PTS_FAIL ;
	}
	else if(pthread_rwlock_unlock_3_1_rd_thread_state != 2)
	{
	...
	pthread_rwlock_unlock_3_1_wr_thread_state_2 = 1;
	priority = sched_get_priority_min(TRD_POLICY);
	printf("main: create writer2, with priority: %d\n", priority);
	if(pthread_create(&writer2, NULL, pthread_rwlock_unlock_3_1_fn_wr_2, (void*)(long)priority) != 0)
	{
	...
	do{
		sleep(1);
	}while (pthread_rwlock_unlock_3_1_wr_thread_state_2 != 3 && cnt++ < 2); 
	
	if(pthread_rwlock_unlock_3_1_wr_thread_state_2 == 3)
	{
		printf("writer2 did not block on write lock, when main owns the lock\n");
		return PTS_UNRESOLVED ;
	}
	else if(pthread_rwlock_unlock_3_1_wr_thread_state_2 != 2)
	{
		printf("Unexpected writer1 state\n");
		return PTS_UNRESOLVED ;
	}
	...
	if(pthread_rwlock_unlock_3_1_wr_thread_state_1 == 2 )
	{
		printf("Test fail: writer did not get write lock, when main release the lock\n");
		return PTS_FAIL ;
	}
	else if(pthread_rwlock_unlock_3_1_wr_thread_state_1 != 3)
	{
		printf("Unexpected writer1 state\n");
		return PTS_UNRESOLVED ; 
	}
	...
	pthread_rwlock_unlock_3_1_wr_thread_state_1 = 4;
	...
	do{
		sleep(1);
	}while (pthread_rwlock_unlock_3_1_rd_thread_state !=3 && cnt++ < 3); 
	
	if(pthread_rwlock_unlock_3_1_rd_thread_state == 2 )
	{
		printf("Test failed: reader did not get the lock when writer1 release the lock\n");
		return PTS_FAIL ;
	}
	else if(pthread_rwlock_unlock_3_1_rd_thread_state != 3)
	{
		printf("Unexpected reader state\n");
		return PTS_UNRESOLVED ;
	}
	...
	pthread_rwlock_unlock_3_1_rd_thread_state = 4;
	...
	do{
		sleep(1);
	}while (pthread_rwlock_unlock_3_1_wr_thread_state_2 !=3 && cnt++ < 3); 
	
	if(pthread_rwlock_unlock_3_1_wr_thread_state_2 == 2 )
	{
		printf("Test fail: writer2 still blocked on write lock, when main release the lock\n");
		return PTS_FAIL ;
	}
	else if(pthread_rwlock_unlock_3_1_wr_thread_state_2 != 3)
	{
		printf("Unexpected writer2 state\n");
		return PTS_UNRESOLVED ;
	}

	pthread_rwlock_unlock_3_1_wr_thread_state_2 = 4;
	...
	if(pthread_rwlock_destroy(&pthread_rwlock_unlock_3_1_rwlock) != 0)
	{
