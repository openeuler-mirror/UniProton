...
int pthread_attr_getscope_1_1_verify_scope(pthread_attr_t *attr, int scopetype) {
	int rc;
	int scope;
...
int pthread_attr_getscope_1_1()
{
	...
	if( rc != 0) {
        ...
        pthread_attr_getscope_1_1_verify_scope(&attr, SYSTEMSCOPE);
	}
  	rc = pthread_attr_setscope(&attr, PROCESSSCOPE);
	if( rc != 0) {
		perror(ERROR_PREFIX "PTHREAD_SCOPE_SYSTEM is not supported");
	} else {
        pthread_attr_getscope_1_1_verify_scope(&attr, PROCESSSCOPE);
	}
