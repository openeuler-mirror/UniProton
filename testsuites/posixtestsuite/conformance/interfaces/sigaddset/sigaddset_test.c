...
int TEST_sigaddset_1(void)
{
    ...
    for (int i = 0; i <= NUMSIGNALS - 1; i++) {
        if (sigaddset(&signalset, siglist[i]) != 0) {
            printf("TEST_sigfillset_1 signo(%d) add fail.\n", siglist[i]);
            return PTS_FAIL;
        }
    }
    ...
    for (int i = NUMSIGNALS - 1; i >= 0; i--) {
        if (sigismember(&signalset, siglist[i]) == 0) {
            printf("TEST_sigfillset_1 signo(%d) is not member.\n", siglist[i]);
            test_failed = 1;
        }
    }
