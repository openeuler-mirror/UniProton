...
static char sem_post_8_1_semname[20];
static char sem_post_8_1_semname_1[20]; /* Used to record state */

/* set my schedule priority to @priority */
int sem_post_8_1_set_my_prio(int priority)
{
...
int sem_post_8_1_child_fn(int priority, int id)
{
	sem_t *sem, *sem_1;
	if (sem_post_8_1_set_my_prio(priority) == -1)
		return -1 ;
	sem = sem_open(sem_post_8_1_semname, 0);
	if( sem == SEM_FAILED || sem == NULL ) {
		perror(ERROR_PREFIX "sem_open: sem");
		return -1;
	}
	sem_1 = sem_open(sem_post_8_1_semname_1, 0);
...
int sem_post_8_1() 
{
	...
	snprintf(sem_post_8_1_semname, 20, "/" TEST "_%d", getpid());
	/* Initial value of Semaphore is 1 */
	sem = sem_open(sem_post_8_1_semname, O_CREAT, 0777, 1);
	if( sem == SEM_FAILED || sem == NULL ) {
		perror(ERROR_PREFIX "sem_open");
		return PTS_UNRESOLVED;
	}
	
	snprintf(sem_post_8_1_semname_1, 20, "/" TEST "_%d_1", getpid());
	sem_1 = sem_open(sem_post_8_1_semname_1, O_CREAT, 0777, val);
	if( sem_1 == SEM_FAILED || sem_1 == NULL ) {
		perror(ERROR_PREFIX "sem_open: sem_1");
		sem_unlink(sem_post_8_1_semname);
		return PTS_UNRESOLVED;
	}
	...
	if (c_1 == 0)
	{
		...
		sem_post_8_1_child_fn(priority - 2, 1);
	} 
	else if (c_1 < 0)
	{
	...
	{
		/* Child 2 */
		sem_post_8_1_child_fn(priority - 1, 2);
	}
	else if (c_2 < 0)
	{
		perror("Error at fork");	
	...
	*/
	...
	{
		/* Child 3 */
		sem_post_8_1_child_fn(priority - 1, 3);
	}
...
clean_up:
	...
	sem_close(sem_1);
	sem_unlink(sem_post_8_1_semname);
	sem_unlink(sem_post_8_1_semname_1);
	return retval;
}
