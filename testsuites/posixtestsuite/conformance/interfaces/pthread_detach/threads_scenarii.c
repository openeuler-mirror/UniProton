...
static struct __pthread_detach_scenario
{
...
#define NSCENAR (sizeof(pthread_detach_scenarii) / sizeof(pthread_detach_scenarii[0]))

/* This function will initialize every pthread_attr_t object in the pthread_detach_scenarii array */
static void pthread_detach_scenar_init()
{
	int ret=0;
	int i;
	int old;
	...
	#if VERBOSE > 0
	pthread_detach_output("System abilities:\n");
	pthread_detach_output(" TSA: %li\n", tsa);
	pthread_detach_output(" TSS: %li\n", tss);
	pthread_detach_output(" TPS: %li\n", tps);
	pthread_detach_output(" pagesize: %li\n", pagesize);
	pthread_detach_output(" min stack size: %li\n", minstacksize);
	#endif
	...
	{
		#if VERBOSE > 2
		pthread_detach_output("Initializing attribute for scenario %i: %s\n", i, pthread_detach_scenarii[i].descr);
		#endif
		
		ret = pthread_attr_init(&pthread_detach_scenarii[i].ta);
		if (ret != 0)  {  UNRESOLVED(ret, "Failed to initialize a thread attribute object");  }
		
		/* Set the attributes according to the scenario */
		if (pthread_detach_scenarii[i].detached == 1)
		{
			ret = pthread_attr_setdetachstate(&pthread_detach_scenarii[i].ta, PTHREAD_CREATE_DETACHED);
			if (ret != 0)  {  UNRESOLVED(ret, "Unable to set detachstate");  }
		}
		else
		{
			ret = pthread_attr_getdetachstate(&pthread_detach_scenarii[i].ta, &old);
		...
		#if VERBOSE > 4
		pthread_detach_output("Detach state was set sucessfully\n");
		#endif
		
		/* Sched related attributes */
		if (tps>0) /* This routine is dependent on the Thread Execution Scheduling option */
		{
			if (pthread_detach_scenarii[i].explicitsched == 1)
				ret = pthread_attr_setinheritsched(&pthread_detach_scenarii[i].ta, PTHREAD_EXPLICIT_SCHED);
			else
				ret = pthread_attr_setinheritsched(&pthread_detach_scenarii[i].ta, PTHREAD_INHERIT_SCHED);
			if (ret != 0)  {  UNRESOLVED(ret, "Unable to set inheritsched attribute");  }
			#if VERBOSE > 4
			pthread_detach_output("inheritsched state was set sucessfully\n");
			#endif
		}
		#if VERBOSE > 4
		else
			pthread_detach_output("TPS unsupported => inheritsched parameter untouched\n");
		#endif
		
		if (tps>0) /* This routine is dependent on the Thread Execution Scheduling option */
		{
			if (pthread_detach_scenarii[i].schedpolicy == 1)
			{
				ret = pthread_attr_setschedpolicy(&pthread_detach_scenarii[i].ta, SCHED_FIFO);
			}
			if (pthread_detach_scenarii[i].schedpolicy == 2)
			{
				ret = pthread_attr_setschedpolicy(&pthread_detach_scenarii[i].ta, SCHED_RR);
			}
			if (ret != 0)  {  UNRESOLVED(ret, "Unable to set the sched policy");  }
			#if VERBOSE > 4
			if (pthread_detach_scenarii[i].schedpolicy)
				pthread_detach_output("Sched policy was set sucessfully\n");
			else
				pthread_detach_output("Sched policy untouched\n");
			#endif
		}
		#if VERBOSE > 4
		else
			pthread_detach_output("TPS unsupported => sched policy parameter untouched\n");
		#endif
		
		if (pthread_detach_scenarii[i].schedparam != 0)
		{
			struct sched_param sp;
			
			ret = pthread_attr_getschedpolicy(&pthread_detach_scenarii[i].ta, &old);
			if (ret != 0)  {  UNRESOLVED(ret, "Unable to get sched policy from attribute"); }
			
			if (pthread_detach_scenarii[i].schedparam == 1)
				sp.sched_priority = sched_get_priority_max(old);
			if (pthread_detach_scenarii[i].schedparam == -1)
				sp.sched_priority = sched_get_priority_min(old);
			
			ret = pthread_attr_setschedparam(&pthread_detach_scenarii[i].ta, &sp);
			if (ret != 0)  {  UNRESOLVED(ret, "Failed to set the sched param");  }
			
		#if VERBOSE > 4
			pthread_detach_output("Sched param was set sucessfully to %i\n", sp.sched_priority);
		}
		else
		{
			pthread_detach_output("Sched param untouched\n");
		#endif
		}
		
		if (tps>0) /* This routine is dependent on the Thread Execution Scheduling option */
		{
			ret = pthread_attr_getscope(&pthread_detach_scenarii[i].ta, &old);
			if (ret != 0)  {  UNRESOLVED(ret, "Failed to get contension scope from thread attribute");  }
			
			if (pthread_detach_scenarii[i].altscope != 0)
			{
				...
				ret = pthread_attr_setscope(&pthread_detach_scenarii[i].ta, old);
				//if (ret != 0)  {  UNRESOLVED(ret, "Failed to set contension scope");  }
				#if VERBOSE > 0
				if (ret != 0)  {  pthread_detach_output("WARNING: The TPS option is claimed to be supported but setscope fails\n");  }
				#endif
				
			#if VERBOSE > 4
				pthread_detach_output("Contension scope set to %s\n", old==PTHREAD_SCOPE_PROCESS?"PTHREAD_SCOPE_PROCESS":"PTHREAD_SCOPE_SYSTEM");
			}
			else
			{
				pthread_detach_output("Contension scope untouched (%s)\n", old==PTHREAD_SCOPE_PROCESS?"PTHREAD_SCOPE_PROCESS":"PTHREAD_SCOPE_SYSTEM");
			#endif
			}
		}
		#if VERBOSE > 4
		else
			pthread_detach_output("TPS unsupported => sched contension scope parameter untouched\n");
		#endif
		...
		{
			if (pthread_detach_scenarii[i].altstack != 0)
			{
				...
				pthread_detach_scenarii[i].bottom = malloc(minstacksize + pagesize);
				if (pthread_detach_scenarii[i].bottom == NULL)  {  UNRESOLVED(errno,"Unable to alloc enough memory for alternative stack"); }
				
				ret = pthread_attr_setstack(&pthread_detach_scenarii[i].ta, pthread_detach_scenarii[i].bottom, minstacksize);
				if (ret != 0)  {  UNRESOLVED(ret, "Failed to specify alternate stack");  }
			
				#if VERBOSE > 1
				pthread_detach_output("Alternate stack created successfully. Bottom=%p, Size=%i\n", pthread_detach_scenarii[i].bottom, minstacksize);
				#endif
			}
		}
		#if VERBOSE > 4
		else
			pthread_detach_output("TSA or TSS unsupported => No alternative stack\n");
		#endif
		
		#ifndef WITHOUT_XOPEN
		if (pthread_detach_scenarii[i].guard != 0)
		{
			if (pthread_detach_scenarii[i].guard == 1)
				ret = pthread_attr_setguardsize(&pthread_detach_scenarii[i].ta, 0);
			if (pthread_detach_scenarii[i].guard == 2)
				ret = pthread_attr_setguardsize(&pthread_detach_scenarii[i].ta, pagesize);
			if (ret != 0)  {  UNRESOLVED(ret, "Unable to set guard area size in thread stack");  }
			#if VERBOSE > 4
			pthread_detach_output("Guard size set to %i\n", (pthread_detach_scenarii[i].guard==1)?1:pagesize);
			#endif
		}
		#endif
		...
		{
			if (pthread_detach_scenarii[i].altsize != 0)
			{
				ret = pthread_attr_setstacksize(&pthread_detach_scenarii[i].ta, minstacksize);
				if (ret != 0)  {  UNRESOLVED(ret, "Unable to change stack size");  }
				#if VERBOSE > 4
				pthread_detach_output("Stack size set to %i (this is the min)\n", minstacksize);
				#endif
			}
		}
		#if VERBOSE > 4
		else
			pthread_detach_output("TSS unsupported => stack size unchanged\n");
		#endif

		ret = sem_init(&pthread_detach_scenarii[i].sem, 0,0);
		if (ret == -1) {  UNRESOLVED(errno, "Unable to init a semaphore");  }
		
	}
	#if VERBOSE > 0
	pthread_detach_output("All %i thread attribute objects were initialized\n\n", NSCENAR);
	#endif
}

/* This function will free all resources consumed in the pthread_detach_scenar_init() routine */
static void pthread_detach_scenar_fini(void)
{
	int ret = 0, i;
	
	for (i=0; i<NSCENAR; i++)
	{
		if (pthread_detach_scenarii[i].bottom != NULL)
			free(pthread_detach_scenarii[i].bottom);
		
		ret = sem_destroy(&pthread_detach_scenarii[i].sem);
		if (ret == -1) {  UNRESOLVED(errno, "Unable to destroy a semaphore");  }
		
		ret = pthread_attr_destroy(&pthread_detach_scenarii[i].ta);
		if (ret != 0)  {  UNRESOLVED(ret, "Failed to destroy a thread attribute object");  }
	}
}

static int pthread_detach_sc=0; /* This might be very dirty... but is much simpler */
...
int RUN_MAIN (int argc, char *argv[])
{
	...
	for (pthread_detach_sc=0; pthread_detach_sc < NSCENAR; pthread_detach_sc++)
	{
		...
		ret = pthread_create(&child, &pthread_detach_scenarii[pthread_detach_sc].ta, threaded, NULL);
