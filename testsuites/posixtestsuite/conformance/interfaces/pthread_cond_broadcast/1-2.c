...
void * pthread_cond_broadcast_1_2_child(void * arg)
{
	int ret=0;
	struct timespec ts;
	char timed;
...
void * pthread_cond_broadcast_1_2_timer(void * arg)
{
...
int pthread_cond_broadcast_1_2(int argc, char * argv[])
{
	int ret;
 ...
 */
	...
	for (scenar=0; scenar < NSCENAR; scenar++)
	{
		...
		ret = pthread_create(&t_timer, NULL, pthread_cond_broadcast_1_2_timer, NULL);
		...
		if (td->fork==0)
		{
			do
			{
				...
				{
					ret = pthread_create(&(tmp->data.t), &ta, pthread_cond_broadcast_1_2_child, NULL);
					if (ret != 0)
					{
						free(tmp);
					}
		...
		else
		{
			do
			{
				...
				if (tmp != (children_t *) NULL)
				{
					...
					{
                        pthread_cond_broadcast_1_2_child(NULL);
						exit(0);
					}
