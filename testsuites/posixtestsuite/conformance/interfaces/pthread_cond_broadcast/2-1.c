...
void *pthread_cond_broadcast_2_1_thr_func(void *arg)
{
	int rc;
...
int pthread_cond_broadcast_2_1()
{
	int i, rc;
	...
	for (i=0; i<THREAD_NUM; i++) {  /* create THREAD_NUM threads */
	    	if (pthread_create(&thread[i], NULL, pthread_cond_broadcast_2_1_thr_func, NULL) != 0) {
