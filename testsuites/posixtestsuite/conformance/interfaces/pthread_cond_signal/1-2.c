...
void * pthread_cond_signal_1_2_child(void * arg)
{
	int ret=0;
	struct timespec ts;
	char timed;
...
/* Timeout thread */
void * pthread_cond_signal_1_2_timer(void * arg)
{
...
int pthread_cond_signal_1_2(int argc, char * argv[])
{
	int ret;
 ...
 */
	...
	for (scenar=0; scenar < NSCENAR; scenar++)
	{
		...
		for (ch=0; ch < NTHREADS; ch++)
		{
			if (td->fork==0)
			{
				ret = pthread_create(&t_child[ch], NULL, pthread_cond_signal_1_2_child, NULL);
			...
			else
			{
				...
				{
                    pthread_cond_signal_1_2_child(NULL);
					exit(0);
				}
			}
		}
		...
		ret = pthread_create(&t_timer, NULL, pthread_cond_signal_1_2_timer, p_child);
