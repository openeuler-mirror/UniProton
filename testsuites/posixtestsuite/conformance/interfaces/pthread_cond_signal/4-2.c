...
void * pthread_cond_signal_4_2_sendsig (void * arg)
{
...
void pthread_cond_signal_4_2_sighdl1(int sig)
{
#ifdef WITH_SYNCHRO
	if (sem_post(&semsig1))
...
void pthread_cond_signal_4_2_sighdl2(int sig)
{
#ifdef WITH_SYNCHRO
	if (sem_post(&semsig2))
...
void * pthread_cond_signal_4_2_waiter(void * arg)
{
	int ret;
...
void * pthread_cond_signal_4_2_worker (void * arg)
{
	int ret=0;
...
/* Main function */
int pthread_cond_signal_4_2(int argc, char * argv[])
{
	int ret;
    ...
    pthread_cond_signal_output_init();
	
	/* We need to register the signal handlers for the PROCESS */
	sigemptyset (&sa.sa_mask);
	sa.sa_flags = 0;
	sa.sa_handler = pthread_cond_signal_4_2_sighdl1;
	if ((ret = sigaction (SIGUSR1, &sa, NULL)))
	{ UNRESOLVED(ret, "Unable to register signal handler1"); }
	sa.sa_handler = pthread_cond_signal_4_2_sighdl2;
	...
	if ((ret = pthread_create(&th_waiter, NULL, pthread_cond_signal_4_2_waiter, NULL)))
	{ UNRESOLVED(ret, "Waiter thread creation failed"); }
	
	if ((ret = pthread_create(&th_worker, NULL, pthread_cond_signal_4_2_worker, NULL)))
	...
	if ((ret = pthread_create(&th_sig1, NULL, pthread_cond_signal_4_2_sendsig, (void *)&arg1)))
	{ UNRESOLVED(ret, "Signal 1 sender thread creation failed"); }
	if ((ret = pthread_create(&th_sig2, NULL, pthread_cond_signal_4_2_sendsig, (void *)&arg2)))
