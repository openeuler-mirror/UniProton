...
void * pthread_cond_timedwait_2_7_tf(void * arg)
{
	int ret=0;
	struct timespec ts;
...
int pthread_cond_timedwait_2_7(int argc, char * argv[])
{
	int ret, i;
 ...
 */
		...
		if (do_fork != 0)
		{
			...
			if (child_pr == 0)
			{
				...
				if (pthread_cond_timedwait_2_7_tf((void *)td) != NULL)
				{
		...
		else /* do_fork == 0 */
		{
			...
			ret = pthread_create(&child_th, NULL, pthread_cond_timedwait_2_7_tf, td);
