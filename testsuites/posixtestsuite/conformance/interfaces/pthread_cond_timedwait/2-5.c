...
void * pthread_cond_timedwait_2_5_threaded ( void * arg )
{
	int ret;
...
int pthread_cond_timedwait_2_5(int argc, char * argv[])
{
	int ret, i, j;
	...
	for (i=0; i< (sizeof(scenar) / sizeof(scenar[0])); i++)
	{
		...
		{
			ret = pthread_create(&th[j], NULL, pthread_cond_timedwait_2_5_threaded, (void *)(long)(j & 1));
