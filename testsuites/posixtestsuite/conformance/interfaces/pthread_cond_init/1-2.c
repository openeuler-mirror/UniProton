 ...
 */
void * pthread_cond_init_1_2_test_timeout(void * arg)
{
	int ret;
 ...
 */
int pthread_cond_init_1_2_do_cs_test(void)
{
	int ret;
	int result=0;
	...
	else /* We can do the test */
	{
		...
		/* create the threads */
		ret = pthread_create(&thD, NULL, pthread_cond_init_1_2_test_timeout, &dtD);
		if (ret != 0)
		{  UNRESOLVED(ret, "Unable to create a thread");  }
		ret = pthread_create(&thN, NULL, pthread_cond_init_1_2_test_timeout, &dtN);
		if (ret != 0)
		{  UNRESOLVED(ret, "Unable to create a thread");  }
 ...
 * Main function
 */
int pthread_cond_init_1_2(int argc, char * argv[])
{
	long opt_TMR, opt_CS;
	int ret=0;
	...
	RttViewerPrintf(0, "-----pthread_cond_init_1_2 opt_TMR =%d, opt_CS=%d ---\n ", opt_TMR, opt_CS);
	...
	if ((opt_TMR != -1L) && (opt_CS != -1L))
	{
		...
		#endif
		ret = pthread_cond_init_1_2_do_cs_test();
	}
	else
	{
