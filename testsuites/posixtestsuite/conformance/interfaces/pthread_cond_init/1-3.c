 ...
 * 
 */
void pthread_cond_init_1_3_childsighdl(int sig)
{
	int ret=0;
 ...
 *
 */
void pthread_cond_init_1_3_mainsighdl(int sig)
{
 ...
 *  
 */
void * pthread_cond_init_1_3_child_process_th(void * arg)
{
	int ret=0;
	...
	*(dt->pth) = pthread_self();
	...
	sa.sa_flags = 0;
	sa.sa_handler = pthread_cond_init_1_3_childsighdl;
 ...
 */
int pthread_cond_init_1_3_child_process(globaldata_t * gd)
{
	int ret;
	...
	sa.sa_flags = 0;
	sa.sa_handler = pthread_cond_init_1_3_mainsighdl;
	...
	ret = pthread_create(&thN, NULL, pthread_cond_init_1_3_child_process_th, &dtN);
	if (ret != 0)
	{  UNRESOLVED(ret, "[child] Unable to create thread N");  }
	
	ret = pthread_create(&thD, NULL, pthread_cond_init_1_3_child_process_th, &dtD);
	if (ret != 0)
	{  UNRESOLVED(ret, "[child] Unable to create thread D");  }

	ret = pthread_create(&thP, NULL, pthread_cond_init_1_3_child_process_th, &dtP);
	if (ret != 0)
...
int pthread_cond_init_1_3_parent_process(globaldata_t * gd)
{
	int ret, tmp, i;
 ...
 */
int pthread_cond_init_1_3_do_tps_test(void)
{
	int ret=0;
	globaldata_t * gd;
	...
	{
		/* We are the child */
		ret = pthread_cond_init_1_3_child_process(gd);
		#if VERBOSE > 1
	...
	/* We are the parent */
	gd->child = child;
	rc2 = pthread_cond_init_1_3_parent_process(gd);
	#if VERBOSE > 1
 ...
 */
int pthread_cond_init_1_3(int argc, char * argv[])
{
	long opt_TPS, opt_MF;
	int ret=0;
	...
	if ((opt_TPS != -1L) && (opt_MF != -1L))
	{
		...
		#endif
		ret = pthread_cond_init_1_3_do_tps_test();
	}
	else
	{
