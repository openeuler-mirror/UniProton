...
int pthread_cond_init_4_1(int argc, char * argv[])
{
	pid_t child;
	...
	#if VERBOSE > 1
    pthread_cond_init_output("Ready to take over memory. Page size is %d\n", sz);
	#endif
	...
	#if VERBOSE > 1
    pthread_cond_init_output("%d pages were allocated before failure\n", ret);
	ret = 0;
	#endif
	...
	#if VERBOSE > 1
    pthread_cond_init_output("%d additional spaces were allocated before failure\n", ret);
	ret = 0;
	#endif
	if (errno != ENOMEM)
	...
	#if VERBOSE > 1
    pthread_cond_init_output("Memory is released\n");
	#endif
	...
	for (i=0; i<5; i++)
	{
		...
		{
			#if VERBOSE > 0
            pthread_cond_init_output("Cond (%i) initialization succeeded when memory is full\n", i);
			#endif
			if (retdtr[i] != 0)
		...
		{
            pthread_cond_init_output("Cond (%i) initialization failed with ENOMEM\n", i);
		}
		#endif
	}
	PASSED;
