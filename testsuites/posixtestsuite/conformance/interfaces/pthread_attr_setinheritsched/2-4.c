...
int pthread_attr_setinheritsched_2_4_policy_correct = -1;
int pthread_attr_setinheritsched_2_4_priority_correct = -1;

void *pthread_attr_setinheritsched_2_4_thread(void *tmp)
{
	...
	if(policy == POLICY) {
		pthread_attr_setinheritsched_2_4_policy_correct = 1;
	}
	if(param.sched_priority == PRIORITY) {
		pthread_attr_setinheritsched_2_4_priority_correct = 1;
	}

	return NULL;
}

int pthread_attr_setinheritsched_2_4()
{
	pthread_attr_t             attr;
	pthread_t                  thread_id;
	struct sched_param         param;
	...
	rc = pthread_create(&thread_id, &attr, pthread_attr_setinheritsched_2_4_thread, NULL);
	if(rc != 0) {
	...
	if((pthread_attr_setinheritsched_2_4_priority_correct != 1) || (pthread_attr_setinheritsched_2_4_policy_correct != 1)) {
		printf("Test FAIL\n");
		return PTS_FAIL ;
	}
