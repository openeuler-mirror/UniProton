 ...
 *      -> Is an error pthread_mutex_init_1_2_returned when unlocking the mutex in unlocked state? 
...
/**** global variables ****/
pthread_mutex_t * pthread_mutex_init_1_2_p_mtx;
int pthread_mutex_init_1_2_retval = 0;
int pthread_mutex_init_1_2_returned = 0;
int pthread_mutex_init_1_2_canceled = 0;
sem_t pthread_mutex_init_1_2_semA, pthread_mutex_init_1_2_semB;

/***** Cancelation handlers  *****/
void pthread_mutex_init_1_2_cleanup_deadlk(void * arg)
{
	pthread_mutex_init_1_2_canceled = 1;
	pthread_mutex_unlock(pthread_mutex_init_1_2_p_mtx);	
}

/***** Threads functions *****/
void * pthread_mutex_init_1_2_deadlk_issue(void * arg)
{
	int ret, tmp;
	
	if ((ret=pthread_mutex_lock(pthread_mutex_init_1_2_p_mtx)))
	{ UNRESOLVED(ret, "First mutex lock in pthread_mutex_init_1_2_deadlk_issue"); }
	pthread_cleanup_push(pthread_mutex_init_1_2_cleanup_deadlk, NULL);
	if ((ret = pthread_setcanceltype(PTHREAD_CANCEL_ASYNCHRONOUS, &tmp)))
	{ UNRESOLVED(ret, "Set cancel type in pthread_mutex_init_1_2_deadlk_issue"); }
	if ((ret = pthread_setcancelstate(PTHREAD_CANCEL_ENABLE, &tmp)))
	{ UNRESOLVED(ret, "Set cancel state in pthread_mutex_init_1_2_deadlk_issue"); }
	#if VERBOSE >1
	pthread_mutex_init_output("Thread releases the semaphore...\n");
	#endif
	if ((ret = sem_post(&pthread_mutex_init_1_2_semA)))
	{ UNRESOLVED(errno, "Sem_post in pthread_mutex_init_1_2_deadlk_issue"); }
    
    	pthread_mutex_init_1_2_returned = 0;
	pthread_mutex_init_1_2_retval = pthread_mutex_lock(pthread_mutex_init_1_2_p_mtx);
	pthread_mutex_init_1_2_returned = 1;
...
void * pthread_mutex_init_1_2_unlock_issue(void * arg)
{
	int ret;
	...
	#endif
	if ((ret=pthread_mutex_lock(pthread_mutex_init_1_2_p_mtx)))
	{ UNRESOLVED(ret, "First mutex lock in pthread_mutex_init_1_2_unlock_issue"); }

	if ((ret = sem_post(&pthread_mutex_init_1_2_semA)))
	{ UNRESOLVED(errno, "Sem_post in pthread_mutex_init_1_2_unlock_issue"); }

	if ((ret = sem_wait(&pthread_mutex_init_1_2_semB)))
	{ UNRESOLVED(errno, "Sem_wait in pthread_mutex_init_1_2_unlock_issue"); }

	if (pthread_mutex_init_1_2_retval != 0) /* parent thread failed to unlock the mutex) */
	{
		#if VERBOSE >1
		pthread_mutex_init_output("Unlocking in child...\n");
		#endif
		if ((ret=pthread_mutex_unlock(pthread_mutex_init_1_2_p_mtx)))
		{ FAILED("Mutex unlock pthread_mutex_init_1_2_returned an error but mutex is unlocked."); }
	}
...
/***** main program *****/
int pthread_mutex_init_1_2(int argc, char *argv[])
{
	...
	if ((ret=sem_init(&pthread_mutex_init_1_2_semA, 0, 0)))
	{ UNRESOLVED(errno, "Sem A init"); }
	if ((ret=sem_init(&pthread_mutex_init_1_2_semB, 0, 0)))
	...
	pthread_mutex_init_1_2_retval = pthread_mutex_unlock(tab_mutex[0]);
	ret = pthread_mutex_unlock(tab_mutex[1]);
	#if VERBOSE >0
	pthread_mutex_init_output("Results for unlock issue #1:\n mutex 1 unlocking pthread_mutex_init_1_2_returned %i\n mutex 2 unlocking pthread_mutex_init_1_2_returned %i\n",
				pthread_mutex_init_1_2_retval, ret);
	#endif
	if (ret != pthread_mutex_init_1_2_retval)
	{
	...
	for (i=0; i<2; i++)
	{
		pthread_mutex_init_1_2_p_mtx = tab_mutex[i];
		tab_res[i][0]=0;
		tab_res[i][1]=0;
		tab_res[i][2]=0;
		...
		if ((ret = pthread_create(&thr, NULL, pthread_mutex_init_1_2_unlock_issue, NULL)))
		{ UNRESOLVED(ret, "Unlock issue thread create"); }
		
		if ((ret = sem_wait(&pthread_mutex_init_1_2_semA)))
		{ UNRESOLVED(errno, "Sem A wait failed for unlock issue."); }
	
		#if VERBOSE >1
		pthread_mutex_init_output("Unlocking in parent...\n");
		#endif
		pthread_mutex_init_1_2_retval = pthread_mutex_unlock(pthread_mutex_init_1_2_p_mtx);
		
		if ((ret = sem_post(&pthread_mutex_init_1_2_semB)))
		...
		tab_res[i][0] = pthread_mutex_init_1_2_retval;
	}
	#if VERBOSE >0
	pthread_mutex_init_output("Results for unlock issue #2:\n mutex 1 pthread_mutex_init_1_2_returned %i\n mutex 2 pthread_mutex_init_1_2_returned %i\n",
	...
	for (i=0; i<2; i++)
	{
		...
		if ((ret = pthread_create(&thr, NULL, pthread_mutex_init_1_2_deadlk_issue, NULL)))
		...
		pthread_mutex_init_1_2_canceled=0;
		#if VERBOSE >1
		pthread_mutex_init_output("Cancel thread...\n");
		#endif
		if (pthread_mutex_init_1_2_returned ==0)
			if ((ret=pthread_cancel(thr)))
			{ UNRESOLVED(ret, "Cancel thread (pthread_mutex_init_1_2_deadlk_issue)"); }

		#if VERBOSE >1
		pthread_mutex_init_output("Thread pthread_mutex_init_1_2_canceled...\n");
		#endif
		...
		tab_res[i][2] = pthread_mutex_init_1_2_retval;
		tab_res[i][1] = pthread_mutex_init_1_2_returned;
		tab_res[i][0] = pthread_mutex_init_1_2_canceled;
	}
	...
	pthread_mutex_init_output("Results for deadlock issue:\n mutex 1 \t%s\t%s%i\n mutex 2 \t%s\t%s%i\n",
				...
				tab_res[0][1]?"pthread_mutex_init_1_2_returned " : "did not return ",
				tab_res[0][2],
				tab_res[1][0]?"deadlock" : "no deadlock",
				tab_res[1][1]?"pthread_mutex_init_1_2_returned " : "did not return ",
				tab_res[1][2]);
	#endif
	...
	{ FAILED("The locks pthread_mutex_init_1_2_returned different error codes."); }
