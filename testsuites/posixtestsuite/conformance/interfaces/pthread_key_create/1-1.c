 ...
 *
 * Steps:
 * 1. Define an array of pthread_key_create_1_1_keys
 * 2. Use pthread_key_create() and create those pthread_key_create_1_1_keys
 * 3. Verify that you can set and get specific values for those pthread_key_create_1_1_keys without errors.
 * 
 */
...
pthread_key_t pthread_key_create_1_1_keys[NUM_OF_KEYS];

int pthread_key_create_1_1()
{
	int i;
	void* rc;
	...
	{
		if(pthread_key_create(&pthread_key_create_1_1_keys[i], NULL) != 0)
		{
			printf("Error: pthread_key_create() failed\n");
			return PTS_UNRESOLVED;
		} else
		{
			if(pthread_setspecific(pthread_key_create_1_1_keys[i], (void *)(long)(i + KEY_VALUE)) != 0)
			{
	...
	for(i = 0;i<NUM_OF_KEYS;++i)
	{
		rc = pthread_getspecific(pthread_key_create_1_1_keys[i]);
		...
		{
			if(pthread_key_delete(pthread_key_create_1_1_keys[i]) != 0)
			{
