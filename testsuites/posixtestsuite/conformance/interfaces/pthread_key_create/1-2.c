...
pthread_key_t pthread_key_create_1_2_keys[NUM_OF_THREADS];
int pthread_key_create_1_2_i;

/* Thread function that sets the key to KEY_VALUE */
void *pthread_key_create_1_2_a_thread_func()
{
	/* Set the key to KEY_VALUE */
	if(pthread_setspecific(pthread_key_create_1_2_keys[pthread_key_create_1_2_i], (void *)(KEY_VALUE)) != 0)
	{
...
int pthread_key_create_1_2()
{
	pthread_t new_th;
	void *value_ptr;

	/* Create a key */
	for(pthread_key_create_1_2_i = 0;pthread_key_create_1_2_i<NUM_OF_THREADS;pthread_key_create_1_2_i++)
	{
		if(pthread_key_create(&pthread_key_create_1_2_keys[pthread_key_create_1_2_i], NULL) != 0)
		{
	...
	{
		/* Create a thread */
		if(pthread_create(&new_th, NULL, pthread_key_create_1_2_a_thread_func, NULL) != 0)
		{
		...
		{
			printf("Test FAILED: Could not use a certain key value to set for many pthread_key_create_1_2_keys\n");
			return PTS_FAIL;
		}
	}
