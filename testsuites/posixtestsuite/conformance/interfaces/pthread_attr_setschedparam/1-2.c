...
volatile int pthread_attr_setschedparam_1_2_thread_created = 0;

void *pthread_attr_setschedparam_1_2_thread_func()
{
	pthread_attr_setschedparam_1_2_thread_created = 1;
	pthread_exit(0);
	return (void*)(0);
}

int pthread_attr_setschedparam_1_2()
{
	pthread_t              thread;
	pthread_attr_t         attr;
	void                   *status;
	...
	rc = pthread_create(&thread, &attr, pthread_attr_setschedparam_1_2_thread_func, NULL);
	if(rc != 0) {
		if (rc == EPERM) {
	...
	if(pthread_attr_setschedparam_1_2_thread_created == 0) {
