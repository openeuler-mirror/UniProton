...
int pthread_attr_setschedparam_1_4_policy_correct = -1;	
/* the thread uses this to indicate to main or success */
int pthread_attr_setschedparam_1_4_priority_correct = -1;	


/* Thread function which checks the scheduler settings for itself */
void * pthread_attr_setschedparam_1_4_thread(void *tmp)
{
	...
	if(policy == POLICY) {
		pthread_attr_setschedparam_1_4_policy_correct = 1;
	}
	if(param.sched_priority == PRIORITY) {
		pthread_attr_setschedparam_1_4_priority_correct = 1;
	}
	return NULL;
}

int pthread_attr_setschedparam_1_4()
{
	pthread_t               thread_id;
	pthread_attr_t          attr;
	struct sched_param      param;
	...
	rc = pthread_create(&thread_id, &attr, pthread_attr_setschedparam_1_4_thread, NULL);
	if(rc != 0) {
	...
	/* test the result */
	if(pthread_attr_setschedparam_1_4_priority_correct != 1) {
		printf("Test FAILED. Priority set incorrectly\n");
		return PTS_FAIL;
	}
	if(pthread_attr_setschedparam_1_4_policy_correct != 1) {
