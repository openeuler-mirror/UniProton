...
void * pthread_cond_wait_2_2_tf(void * arg)
{
	int ret=0;
...
int pthread_cond_wait_2_2(int argc, char * argv[])
{
	int ret, i;
 ...
 */
		...
		if (do_fork != 0)
		{
			...
			if (child_pr == 0)
			{
				...
				if (pthread_cond_wait_2_2_tf((void *)td) != NULL)
				{
		...
		else /* do_fork == 0 */
		{
			...
			ret = pthread_create(&child_th, NULL, pthread_cond_wait_2_2_tf, td);
