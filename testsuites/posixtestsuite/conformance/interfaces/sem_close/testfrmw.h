...
#include <string.h> /* for the strerror() routine */
 ...
 { sem_close_output("Test %s unresolved: got %i (%s) on line %i (%s)\n", __FILE__, x, strerror(x), __LINE__, s); \
 	sem_close_output_fini(); \
 	return PTS_UNRESOLVED ; }
 	
 #define FAILED(s) \
 { sem_close_output("Test %s FAILED: %s\n", __FILE__, s); \
 	sem_close_output_fini(); \
 	return PTS_FAIL ; }
 ...
 #define UNTESTED(s) \
{	sem_close_output("File %s cannot test: %s\n", __FILE__, s); \
	  sem_close_output_fini(); \
  return PTS_UNTESTED ; \
}
  ...
  #define UNRESOLVED(x, s) \
 { sem_close_output("Test unresolved: got %i (%s) on line %i (%s)\n", x, strerror(x), __LINE__, s); \
  sem_close_output_fini(); \
 	return PTS_UNRESOLVED ; }
 	
 #define FAILED(s) \
 { sem_close_output("Test FAILED: %s\n", s); \
  sem_close_output_fini(); \
 	return PTS_FAIL ; }
 ...
 #define UNTESTED(s) \
{	sem_close_output("Unable to test: %s\n", s); \
	  sem_close_output_fini(); \
  return PTS_UNTESTED ; \
}
