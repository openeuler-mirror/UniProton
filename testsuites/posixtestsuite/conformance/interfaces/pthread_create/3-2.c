...
/* The pthread_create_3_2_overflow function is used to test the stack pthread_create_3_2_overflow */
void * pthread_create_3_2_overflow(void * arg)
{
	void * current;
	...
	if (arg == NULL)
	{
		...
		/* Go to recursion */
		current = pthread_create_3_2_overflow(&current);
		...
		/* Terminate the pthread_create_3_2_overflow thread */
		return current;
	}
	...
	return pthread_create_3_2_overflow(arg);
}
...
*/
int pthread_create_3_2_test_stack(pthread_attr_t * ta, size_t newsize)
{
	pid_t child, ctrl;
	int status;
	int ret;
	...
	if (child == -1)  
	{
		pthread_create_output("Failed to fork (%s)\n", strerror(errno));
		return 2;
	}
	...
	if (child != 0) /* father */
	{
		...
		if (ctrl != child)  
		{
			pthread_create_output("Failed to wait for process termination (%s)\n", strerror(errno));
			return 2;
		}
		...
		if (WIFEXITED(status)) /* The process exited */
		{
			...
			{  
				pthread_create_output("The child process returned unresolved status\n");
				return 2;
			}
			else
			{  
		...
		{
			#if VERBOSE > 4
			pthread_create_output("The child process did not return\n");
	...
	/* this is the new process */
	{
		...
		ret = pthread_create(&th, ta, pthread_create_3_2_overflow, NULL);  /* Create a new thread with the same attributes */
		...
		if (detach == PTHREAD_CREATE_JOINABLE)
		{
			...
			if (rc != (void *)0)
			{  UNRESOLVED((int)(long)rc, "The pthread_create_3_2_overflow function returned an unexpected value");  }
		}
...
} pthread_create_3_2_testdata_t;

void * pthread_create_3_2_schedtest(void * arg)
{
	pthread_create_3_2_testdata_t * td = (pthread_create_3_2_testdata_t *)arg;
	int newpol, ret=0;
	...
	#if VERBOSE > 0
	if (ret != 0)
		pthread_create_output("Changing the current thread sched policy failed with error: %s\n", strerror(ret));
	#endif
	#if VERBOSE > 2
	else
		pthread_create_output("Executing thread scheduling policy changed\n");
	#endif
...
int pthread_create_3_2 (int argc, char *argv[])
{
	int ret=0;
	int do_stack_tests;
	int do_sched_tests;
	...
	do_stack_tests = (pthread_create_3_2_test_stack(NULL,0)==0?1:0);
	#if VERBOSE > 0
	pthread_create_output("Test starting\n Stack tests %s be executed.\n Sched tests %s be executed.\n",
	...
	for (pthread_create_sc=0; pthread_create_sc < NSCENAR; pthread_create_sc++)
	{
	   ...
	   /* stack grow test */
			...
			if ((pthread_create_scenarii[pthread_create_sc].altstack == 0) && (pthread_create_scenarii[pthread_create_sc].altsize == 1))
			{
				#if VERBOSE > 2
				pthread_create_output("Processing stack grow test\n");
				#endif
				
				ret = pthread_create_3_2_test_stack(&pthread_create_scenarii[pthread_create_sc].ta, 2*sysconf(_SC_THREAD_STACK_MIN));
				
				if (ret == 0) 
				{
					if (pthread_create_scenarii[pthread_create_sc].guard == 2)
					{
						FAILED("Changing the stacksize after the thread was created changed the running thread stack size");
					}
					#if VERBOSE > 2
					else
						pthread_create_output("We were able to pthread_create_3_2_overflow the stack, but the guard area is unknow or null\n");
					#endif
					
				}
				
				if ((ret != 2) && (pthread_create_scenarii[pthread_create_sc].result == 1))
				{
				...
				if ((ret == 1))
				{
					pthread_create_output("Stack grow test passed\n");
				}
				
				if ((ret == 2) && (pthread_create_scenarii[pthread_create_sc].result == 2))
				{
					pthread_create_output("Something went wrong -- we don't care in this case\n");
				}
				#endif
				
				if ((ret == 2) && (pthread_create_scenarii[pthread_create_sc].result == 0))
				{
					UNRESOLVED(-1, "An unexpected error occured\n");
				}
				
				/* Ok, set back the thread attribute object to a correct value */
				ret = pthread_attr_setstacksize(&pthread_create_scenarii[pthread_create_sc].ta, sysconf(_SC_THREAD_STACK_MIN));
				if (ret != 0)  {  UNRESOLVED(ret, "Failed to set stacksize back");  }
			}
			
			
	   /* stack decrease test */
			if ((pthread_create_scenarii[pthread_create_sc].altstack == 0) && (pthread_create_scenarii[pthread_create_sc].altsize == 0))
			{
				#if VERBOSE > 2
				pthread_create_output("Processing stack decrease test\n");
				#endif
				
				ret = pthread_create_3_2_test_stack(&pthread_create_scenarii[pthread_create_sc].ta, sysconf(_SC_THREAD_STACK_MIN));
				
				if (ret == 1)  {  FAILED("Decreasing the stack size after thread is created had an influence on the thread");  }
				
				if ((ret == 0) && (pthread_create_scenarii[pthread_create_sc].result == 1))
				{
					UNRESOLVED(-1, "An error was expected but the thread creation succeeded");
				}
				...
				#if VERBOSE > 2
				if (ret == 0)
					pthread_create_output("Stack decrease test passed.\n");
				else
					pthread_create_output("Something failed but we don't care here.\n");
				#endif
			}
	  ...
	  /* Sched policy/param change test */
	  		if (pthread_create_scenarii[pthread_create_sc].explicitsched != 0) /* We need a specified policy */
			{
				pthread_t child;
				int policy_ori, newpol_max;
				struct sched_param param_ori, tmp;
				
				pthread_create_3_2_testdata_t td;
				
				#if VERBOSE > 2
				pthread_create_output("Processing sched policy/param change test\n");
				#endif

				/* Backup the scenario object */
				ret = pthread_attr_getschedpolicy(&(pthread_create_scenarii[pthread_create_sc].ta), &policy_ori);
				if (ret != 0)  {  UNRESOLVED(ret, "Unable to read sched policy from thread attribute");  }
				ret = pthread_attr_getschedparam(&(pthread_create_scenarii[pthread_create_sc].ta), &param_ori);
				...
				ret = pthread_create(&child, &(pthread_create_scenarii[pthread_create_sc].ta), pthread_create_3_2_schedtest, &td);
				if (ret != 0)
				{
					if (pthread_create_scenarii[pthread_create_sc].result == 0)
					{  UNRESOLVED(ret , "Failed to create a thread");  }
					
					#if VERBOSE > 2
					if (pthread_create_scenarii[pthread_create_sc].result == 2)
					{
						pthread_create_output("The thread creation failed -- we don't care\n");
					}
					if (pthread_create_scenarii[pthread_create_sc].result == 1)
					{
						pthread_create_output("The thread creation failed as expected\n");
					}
					#endif
				}
				else /* Thread created */
				{
					...
					#if VERBOSE > 2
					if (pthread_create_scenarii[pthread_create_sc].result == 2)
						pthread_create_output("Thread is created\n");
					#endif
					...
					{
						ret = pthread_attr_setschedpolicy(&(pthread_create_scenarii[pthread_create_sc].ta), SCHED_FIFO);
						newpol_max = sched_get_priority_max(SCHED_FIFO);
					}
					else
					{
						ret = pthread_attr_setschedpolicy(&(pthread_create_scenarii[pthread_create_sc].ta), SCHED_RR);
					...
					ret = pthread_attr_setschedparam(&(pthread_create_scenarii[pthread_create_sc].ta), &tmp);
					...
					ret = pthread_attr_setschedpolicy(&(pthread_create_scenarii[pthread_create_sc].ta), policy_ori);
					if (ret != 0)  {  UNRESOLVED(ret, "Unable to read sched policy from thread attribute");  }
					ret = pthread_attr_setschedparam(&(pthread_create_scenarii[pthread_create_sc].ta), &param_ori);
					if (ret != 0)  {  UNRESOLVED(ret, "Unable to read sched param from thread attribute");  }
					...
					ret = pthread_attr_getschedpolicy(&(pthread_create_scenarii[pthread_create_sc].ta), &(td.policy));
					if (ret != 0)  {  UNRESOLVED(ret, "Unable to read sched policy from thread attribute");  }
					ret = pthread_attr_getschedparam(&(pthread_create_scenarii[pthread_create_sc].ta), &(td.param));
					if (ret != 0)  {  UNRESOLVED(ret, "Unable to read sched param from thread attribute");  }
					...
					#if VERBOSE > 2
					pthread_create_output("Sched policy/param change test passed\n");
					#endif
				} /* thread created */
			}
