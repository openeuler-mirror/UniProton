...
int pthread_create_10_1_eated_thread;	/* Flag indicating that the thread start routine was reached, and
			   therefore, the thread was created. */
int pthread_create_10_1_segfault_flag; 	/* Flag indicating that a segmentation fault occured. */

/* Thread's start routine. */
void *pthread_create_10_1_segfault_a_thread_func()
{
	...
	pthread_create_10_1_eated_thread = 1;
	pthread_exit(0);
	return NULL;
}

/* Signal handler for SIGSEGV (segmentation fault signal) */
void pthread_create_10_1_sig_handler(int sig)
{
...
int pthread_create_10_1()
{
	...
	/* Inializing flags. */
	pthread_create_10_1_segfault_flag = 1;
	pthread_create_10_1_eated_thread = 0;

	/* Set signal handler for SIGSEGV (seg fault) */
	act.sa_handler = pthread_create_10_1_sig_handler;
	act.sa_flags = 0;
	sigaction(SIGSEGV, &act, NULL);
	
	/* Create the thread with the invalid, uninitialized attributes object. */
	pthread_create(&new_th, &inv_attr, pthread_create_10_1_segfault_a_thread_func, NULL);

	/* Should not reach here if process correctly seg-faulted. */
	pthread_create_10_1_segfault_flag = 0;
