...
#define THREAD_NAMEED pthread_create_1_6_threaded
#define RUN_MAIN pthread_create_1_6
...
void * pthread_create_1_6_lp_func(void * arg)
{
	int * ctrl = (int *) arg;
	*ctrl=2;
	return NULL;
...
void * pthread_create_1_6_hp_func(void * arg)
{
	int *ctrl = (int *) arg;
	int dummy=0, i;
	do
...
void * pthread_create_1_6_threaded (void * arg)
{
	int ret = 0;
   ...
   /* Scheduling (priority) tests */
	if ((sysconf(_SC_THREAD_PRIORITY_SCHEDULING) > 0) 
		&& (pthread_create_scenarii[pthread_create_sc].explicitsched != 0) 
		&& (pthread_create_scenarii[pthread_create_sc].schedpolicy != 0)
		&& (pthread_create_scenarii[pthread_create_sc].schedparam == 1))
	{
		...
		if (((pthread_create_scenarii[pthread_create_sc].schedpolicy == 1) && (policy != SCHED_FIFO))
		   || ((pthread_create_scenarii[pthread_create_sc].schedpolicy == 2) && (policy != SCHED_RR)))
		{
			FAILED("The thread is not using the scheduling policy that was required");
		}
		if (((pthread_create_scenarii[pthread_create_sc].schedparam == 1) && (sp.sched_priority != sched_get_priority_max(policy)))
		   || ((pthread_create_scenarii[pthread_create_sc].schedparam ==-1) && (sp.sched_priority != sched_get_priority_min(policy))))
		{
		...
		ret = pthread_attr_getschedpolicy(&pthread_create_scenarii[pthread_create_sc].ta, &policy);
		...
		#if VERBOSE > 1
		pthread_create_output("Starting %i high- and 1 low-priority threads.\n", NCPU);
		#endif
		for (i=0; i<NCPU; i++)
		{
			ret = pthread_create(&hpth[i], &ta, pthread_create_1_6_hp_func, &ctrl);
			if (ret != 0)  {  UNRESOLVED(ret, "Failed to create enough threads");  }
		}
		#if VERBOSE > 5
		pthread_create_output("The %i high-priority threads are running\n", NCPU);
		#endif
		...
		ret = pthread_create(&lpth, &ta, pthread_create_1_6_lp_func, &ctrl);
		if (ret != 0)  {  UNRESOLVED(ret, "Failed to create enough threads");  }
		...
		do
		{
			if (ctrl != 0)
			{
				pthread_create_output("The low priority thread executed. This might be normal if you have more than %i CPU.\n", NCPU + 1);
				FAILED("Low priority thread executed -- the sched parameters are ignored?");
			}
			ret = clock_gettime(CLOCK_REALTIME, &now);
			if (ret != 0)  {  UNRESOLVED(errno, "Failed to read current time");  }
			#if VERBOSE > 5
			pthread_create_output("Time: %d.%09d (to: %d.%09d)\n", now.tv_sec, now.tv_nsec, timeout.tv_sec, timeout.tv_nsec);
			#endif
		}
		...
		#if VERBOSE > 1
		pthread_create_output("The scheduling parameter was set accordingly to the thread attribute.\n");
		#endif
