...
int pthread_create_3_1_sem1;		/* Manual semaphore */

void *pthread_create_3_1_a_thread_func()
{
	/* Indicate to main() that the thread was created. */
	pthread_create_3_1_sem1=INTHREAD;
...
int pthread_create_3_1()
{
	...
	/* Initializing */
	pthread_create_3_1_sem1 = INMAIN;	
	 ...
	 * make it joinable. */
	if(pthread_create(&new_th, &new_attr, pthread_create_3_1_a_thread_func, NULL) != 0)
	{	
		perror("Error creating thread\n");
		return PTS_UNRESOLVED;
	}

	while(pthread_create_3_1_sem1==INMAIN)
		sleep(1);
