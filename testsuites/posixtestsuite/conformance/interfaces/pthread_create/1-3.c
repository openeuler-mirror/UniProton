...
void *pthread_create_1_3_a_thread_function();
void pthread_create_1_3_alarm_handler();

pthread_t a;

int pthread_create_1_3()
{
	
	/* Set the action for SIGALRM to generate an error if it is
	 * reached. This is because if SIGALRM was sent, then the 
	 * test timed out. */
	if (signal(SIGALRM, pthread_create_1_3_alarm_handler) == SIG_ERR)
	{
	...
	/* Create a new thread. */
	if(pthread_create(&a, NULL, pthread_create_1_3_a_thread_function, NULL) != 0)
	{
