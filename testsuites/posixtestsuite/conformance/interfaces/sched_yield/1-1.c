...
volatile int sched_yield_1_1_nb_call = 0;

/* Get the number of CPUs */
int sched_yield_1_1_get_ncpu() {
	int ncpu = -1;
...
#ifdef LINUX
int sched_yield_1_1_set_process_affinity(int cpu)
{
	int retval = -1;
	cpu_set_t cpu_mask;
...
int sched_yield_1_1_set_thread_affinity(int cpu)
{
	int retval = -1;
	cpu_set_t cpu_mask;
...
void * sched_yield_1_1_runner(void * arg) {
	int i=0, nc;
	long result = 0;
#ifdef LINUX        
        sched_yield_1_1_set_thread_affinity(*(int *)arg);
        fprintf(stderr, "%ld bind to cpu: %d\n", pthread_self(), *(int*)arg);
#endif
	
	for(;i<LOOP;i++){
		nc = sched_yield_1_1_nb_call;
		sched_yield();

		/* If the value of sched_yield_1_1_nb_call has not change since the last call
		   of sched_yield, that means that the thread does not 
		   relinquish the processor */
		if(nc == sched_yield_1_1_nb_call) {
			result++;
		}
	}
...
void * sched_yield_1_1_busy_thread(void *arg){
#ifdef LINUX        
        sched_yield_1_1_set_thread_affinity(*(int *)arg);
        fprintf(stderr, "%ld bind to cpu: %d\n", pthread_self(), *(int*)arg);
#endif
        while(1){ 
                sched_yield_1_1_nb_call++;
		sched_yield();
	}
...
void sched_yield_1_1_buzy_process(int cpu){
        struct sched_param param;

#ifdef LINUX        
        /* Bind to a processor */
        sched_yield_1_1_set_process_affinity(cpu);
...
int sched_yield_1_1() {
	...
	for(i=0; i<ncpu-1; i++) {
		...
		if(child_pid[i] == -1){
			...
			sched_yield_1_1_buzy_process(i);
	...
	pthread_attr_init(&attr);
        ...
        thread_cpu = ncpu -1;
	if(pthread_create(&tid, &attr, sched_yield_1_1_busy_thread, &thread_cpu) != 0) {
		perror("An error occurs when calling pthread_create()");
		return PTS_UNRESOLVED;
	}
	
	if(pthread_create(&tid_runner, &attr, sched_yield_1_1_runner, &thread_cpu) != 0) {
		perror("An error occurs when calling pthread_create()");
		return PTS_UNRESOLVED;
	}
