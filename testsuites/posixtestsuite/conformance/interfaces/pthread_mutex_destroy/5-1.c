...
int pthread_mutex_destroy_5_1()
{
	...
	if((rc=pthread_mutex_init(&pthread_mutex_destroy_5_1_mutex, NULL)) != 0) {
		fprintf(stderr,"Fail to initialize pthread_mutex_destroy_5_1_mutex, rc=%d\n",rc);
	...
	if((rc=pthread_mutex_destroy(&pthread_mutex_destroy_5_1_mutex)) != 0) {
		fprintf(stderr,"Fail to destroy pthread_mutex_destroy_5_1_mutex after being unlocked, rc=%d\n",rc);
		printf("Test FAILED\n");
		return PTS_FAIL;
	}
