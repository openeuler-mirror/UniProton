...
struct _pthread_mutex_destroy_5_2_scenar
{
...
pthread_mutex_destroy_5_2_scenarii[] =
{
...
#define NSCENAR (sizeof(pthread_mutex_destroy_5_2_scenarii)/sizeof(pthread_mutex_destroy_5_2_scenarii[0]))


/* Main function */
int pthread_mutex_destroy_5_2 (int argc, char * argv[])
{
	int ret;
	int i;
	pthread_mutex_t mtx;
	...
	for (i=0; i<NSCENAR; i++)
	{
		...
		/* Set the mutex type */
		ret = pthread_mutexattr_settype(&ma[i], pthread_mutex_destroy_5_2_scenarii[i].m_type);
		if (ret != 0)  {  UNRESOLVED(ret, "[parent] Unable to set mutex type");  }

		/* Set the pshared attributes, if supported */
		if ((pshared > 0) && (pthread_mutex_destroy_5_2_scenarii[i].m_pshared != 0))
		{
	...
	for (i=0; i<NSCENAR + 2; i++)
	{
		...
		char * stri;
		if (i<NSCENAR)
			stri = pthread_mutex_destroy_5_2_scenarii[i].descr;
		if (i==NSCENAR)
			stri = def;
		if (i==NSCENAR+1)
			stri = nul;
