...
#include <string.h> /* for the strerror() routine */
 ...
 { pthread_mutex_destroy_output("Test %s unresolved: got %i (%s) on line %i (%s)\n", __FILE__, x, strerror(x), __LINE__, s); \
 	pthread_mutex_destroy_output_fini(); \
 	return PTS_UNRESOLVED ; }
 	
 #define FAILED(s) \
 { pthread_mutex_destroy_output("Test %s FAILED: %s\n", __FILE__, s); \
 	pthread_mutex_destroy_output_fini(); \
 	return PTS_FAIL ; }
 ...
 #define UNTESTED(s) \
{	pthread_mutex_destroy_output("File %s cannot test: %s\n", __FILE__, s); \
	  pthread_mutex_destroy_output_fini(); \
  return PTS_UNTESTED ; \
}
  ...
  #define UNRESOLVED(x, s) \
 { pthread_mutex_destroy_output("Test unresolved: got %i (%s) on line %i (%s)\n", x, strerror(x), __LINE__, s); \
  pthread_mutex_destroy_output_fini(); \
 	return PTS_UNRESOLVED ; }
 	
 #define FAILED(s) \
 { pthread_mutex_destroy_output("Test FAILED: %s\n", s); \
  pthread_mutex_destroy_output_fini(); \
 	return PTS_FAIL ; }
 ...
 #define UNTESTED(s) \
{	pthread_mutex_destroy_output("Unable to test: %s\n", s); \
	  pthread_mutex_destroy_output_fini(); \
  return PTS_UNTESTED ; \
}
