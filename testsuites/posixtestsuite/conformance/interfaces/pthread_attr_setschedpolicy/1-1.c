...
void *pthread_attr_setschedpolicy_1_1_thread_func()
{
	pthread_exit(0);
	return (void*)(0);
}

int pthread_attr_setschedpolicy_1_1()
{
	pthread_t thread;
	pthread_attr_t attr;
    	void *status;
	...
	if((rc=pthread_attr_setschedpolicy(&attr,policy)) != 0) {
    	...
    	if((rc=pthread_create(&thread,&attr,pthread_attr_setschedpolicy_1_1_thread_func,NULL)) != 0) {
  		if (rc == EPERM) {
