...
void *pthread_equal_1_2_a_thread_func()
{
...
int pthread_equal_1_2()
{
	pthread_t new_th1, new_th2;

	/* Create a new thread. */
	if(pthread_create(&new_th1, NULL, pthread_equal_1_2_a_thread_func, NULL) != 0)
	{	
	...
	if(pthread_create(&new_th2, NULL, pthread_equal_1_2_a_thread_func, NULL) != 0)
	{	
		perror("Error creating thread\n");
		return PTS_UNRESOLVED;
	}
