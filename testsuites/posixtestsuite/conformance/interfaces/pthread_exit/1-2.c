...
void * pthread_exit_1_2_threaded (void * arg)
{
	pthread_exit(NULL + 1);
...
int pthread_exit_1_2 (int argc, char *argv[])
{
	int ret=0;
	void * rval;
	pthread_t child;
	...
	for (i=0; i < NSCENAR; i++)
	{
		if (pthread_exit_scenarii[i].detached == 0)
		{
			...
			ret = pthread_create(&child, &pthread_exit_scenarii[i].ta, pthread_exit_1_2_threaded, NULL);
