...
int pthread_exit_3_2_global=0;
int pthread_exit_3_2_tab[4];
pthread_key_t pthread_exit_3_2_tld[3];

#define CLEANUP(n) void pthread_exit_3_2_clnp##n(void * arg)\
{\
	pthread_exit_3_2_tab[pthread_exit_3_2_global]=n; \
	pthread_exit_3_2_global++; \
}
...
/* TLD pthread_exit_3_2_destructor */
void pthread_exit_3_2_destructor(void * arg)
{
	*(int *) arg += pthread_exit_3_2_global;
}

/* Thread routine */
void * pthread_exit_3_2_threaded (void * arg)
{
	int ret = 0;
	
	ret = pthread_setspecific(pthread_exit_3_2_tld[0], (void *)&pthread_exit_3_2_tab[3]);
	if (ret != 0)  {  UNRESOLVED(ret, "Failed to set TLD data");  }
	
	pthread_cleanup_push(pthread_exit_3_2_clnp3, NULL);
	pthread_cleanup_push(pthread_exit_3_2_clnp2, NULL);
	ret = pthread_setspecific(pthread_exit_3_2_tld[1], (void *)&pthread_exit_3_2_tab[3]);
	if (ret != 0)  {  UNRESOLVED(ret, "Failed to set TLD data");  }
	
	pthread_cleanup_push(pthread_exit_3_2_clnp1, NULL);
	ret = pthread_setspecific(pthread_exit_3_2_tld[2], (void *)&pthread_exit_3_2_tab[3]);
	if (ret != 0)  {  UNRESOLVED(ret, "Failed to set TLD data");  }
...
int pthread_exit_3_2 (int argc, char *argv[])
{
	int ret=0;
	void * rval;
	pthread_t child;
	...
	for (j=0; j<3; j++)
	{
		ret = pthread_key_create(&pthread_exit_3_2_tld[j], pthread_exit_3_2_destructor);
	...
	for (i=0; i < NSCENAR; i++)
	{
		if (pthread_exit_scenarii[i].detached == 0)
		{
			...
			for (j=0; j<4; j++)
				pthread_exit_3_2_tab[j]=0;
			pthread_exit_3_2_global=0;
			
			ret = pthread_create(&child, &pthread_exit_scenarii[i].ta, pthread_exit_3_2_threaded, NULL);
			...
			if (ret == 0) /* The new thread is running */
			{
				...
				for (j=0; j<3; j++)
				{
					if ((pthread_exit_3_2_tab[j] != j+1) || (pthread_exit_3_2_tab[3] != 9))
					{
						pthread_exit_output("dump:\ntab[0]=%i\ntab[1]=%i\ntab[2]=%i\ntab[3]=%i\n", pthread_exit_3_2_tab[0], pthread_exit_3_2_tab[1], pthread_exit_3_2_tab[2], pthread_exit_3_2_tab[3]);
	...
	for (j=0; j<3; j++)
	{
		ret = pthread_key_delete(pthread_exit_3_2_tld[j]);
