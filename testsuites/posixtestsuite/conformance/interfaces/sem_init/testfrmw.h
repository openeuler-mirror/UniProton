...
#include <string.h> /* for the strerror() routine */
 ...
 { sem_init_output("Test %s unresolved: got %i (%s) on line %i (%s)\n", __FILE__, x, strerror(x), __LINE__, s); \
 	sem_init_output_fini(); \
 	return PTS_UNRESOLVED ; }
 	
 #define FAILED(s) \
 { sem_init_output("Test %s FAILED: %s\n", __FILE__, s); \
 	sem_init_output_fini(); \
 	return PTS_FAIL ; }
 ...
 #define UNTESTED(s) \
{	sem_init_output("File %s cannot test: %s\n", __FILE__, s); \
	  sem_init_output_fini(); \
  return PTS_UNTESTED ; \
}
  ...
  #define UNRESOLVED(x, s) \
 { sem_init_output("Test unresolved: got %i (%s) on line %i (%s)\n", x, strerror(x), __LINE__, s); \
  sem_init_output_fini(); \
 	return PTS_UNRESOLVED ; }
 	
 #define FAILED(s) \
 { sem_init_output("Test FAILED: %s\n", s); \
  sem_init_output_fini(); \
 	return PTS_FAIL ; }
 ...
 #define UNTESTED(s) \
{	sem_init_output("Unable to test: %s\n", s); \
	  sem_init_output_fini(); \
  return PTS_UNTESTED ; \
}
