...
int pthread_cleanup_push_1_2_sem1;		/* Manual semaphore */
int pthread_cleanup_push_1_2_cleanup_flag;

/* The cleanup handler */
void pthread_cleanup_push_1_2_a_cleanup_func(void *flag_val)	
{
	pthread_cleanup_push_1_2_cleanup_flag = (long)flag_val;
	pthread_cleanup_push_1_2_sem1 = INMAIN;
	return;
}

/* Function that the thread executes upon its creation */
void *pthread_cleanup_push_1_2_a_thread_func()
{
	pthread_setcancelstate(PTHREAD_CANCEL_ENABLE, NULL);
	pthread_setcanceltype(PTHREAD_CANCEL_ASYNCHRONOUS, NULL);
	
	pthread_cleanup_push(pthread_cleanup_push_1_2_a_cleanup_func, (void*) CLEANUP_CALLED);
	
	/* Indicate to main() that the thread has been created. */	
	pthread_cleanup_push_1_2_sem1=INMAIN;
	
	/* Wait until main() has sent out a cancel request, meaning until it
	 * sets pthread_cleanup_push_1_2_sem1==INTHREAD */ 
	while(pthread_cleanup_push_1_2_sem1==INMAIN)
		sleep(1);
...
int pthread_cleanup_push_1_2()
{
	pthread_t new_th;
	void *value_ptr;		/* hold return value of thread from pthread_join */ 

	/* Initializing values */
	pthread_cleanup_push_1_2_sem1=INTHREAD;
	pthread_cleanup_push_1_2_cleanup_flag=CLEANUP_NOTCALLED;
	
	/* Create a new thread. */
	if(pthread_create(&new_th, NULL, pthread_cleanup_push_1_2_a_thread_func, NULL) != 0)
	{	
	 ...
	 * pthread_cleanup_push_1_2_a_thread_func() to set pthread_cleanup_push_1_2_sem1=1.) */
	while(pthread_cleanup_push_1_2_sem1==INTHREAD)
		sleep(1);
	...
	if(pthread_cleanup_push_1_2_cleanup_flag != CLEANUP_CALLED)
	{
