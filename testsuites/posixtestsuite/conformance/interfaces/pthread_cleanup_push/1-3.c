...
int pthread_cleanup_push_1_3_cleanup_flag;

/* Cleanup handler */
void pthread_cleanup_push_1_3_a_cleanup_func(void *flag_val)	
{
	pthread_cleanup_push_1_3_cleanup_flag = (long)flag_val;
	return;
}

/* Function that the thread executes upon its creation */
void *pthread_cleanup_push_1_3_a_thread_func()
{
	pthread_cleanup_push(pthread_cleanup_push_1_3_a_cleanup_func, (void*) CLEANUP_CALLED);
...
int pthread_cleanup_push_1_3()
{
	pthread_t new_th;

	/* Initializing values */
	pthread_cleanup_push_1_3_cleanup_flag = CLEANUP_NOTCALLED;
	
	/* Create a new thread. */
	if(pthread_create(&new_th, NULL, pthread_cleanup_push_1_3_a_thread_func, NULL) != 0)
	{	
	...
	if(pthread_cleanup_push_1_3_cleanup_flag != CLEANUP_CALLED)
	{
