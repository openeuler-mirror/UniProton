 ...
 *  
 * STEPS:
 * 1. Setup a pthread_cancel_1_3_mutex and lock it in main()
 * 2. Create a thread.
 ...
 * 5. Make the thread block on the locked pthread_cancel_1_3_mutex
 * 6. Send out a thread cancel request to the new thread, and unlock the pthread_cancel_1_3_mutex allowing the
 *    thread to continue execution.
 * 7. If the cancel request was honored immediately, the
 *    cleanup handler would have been executed, setting the pthread_cancel_1_3_cleanup_flag to -1, making the
 *    test fail. 
 ...
 *    will continue execution and set the pthread_cancel_1_3_cleanup_flag to -2, failing the test.    
 */
...
int pthread_cancel_1_3_sem1;		/* Manual semaphore */
int pthread_cancel_1_3_cleanup_flag;	/* Flag to indicate the thread's cleanup handler was called */
pthread_mutex_t	pthread_cancel_1_3_mutex = PTHREAD_MUTEX_INITIALIZER;	/* Mutex */


/* Cleanup function that the thread executes when it is canceled.  So if
 * pthread_cancel_1_3_cleanup_flag is 1, it means that the thread was canceled. */
void pthread_cancel_1_3_a_cleanup_func(void *unused)
{
	pthread_cancel_1_3_cleanup_flag=-1;
	return;
}

/* Function that the thread executes upon its creation */
void *pthread_cancel_1_3_a_thread_func()
{
	pthread_setcancelstate(PTHREAD_CANCEL_ENABLE, NULL);
	pthread_setcanceltype(PTHREAD_CANCEL_DEFERRED, NULL);
	
	pthread_cleanup_push(pthread_cancel_1_3_a_cleanup_func,NULL);
	
	/* Indicate to main() that the thread has been created. */	
	pthread_cancel_1_3_sem1=INMAIN;

	/* Lock the pthread_cancel_1_3_mutex. It should have already been locked in main, so the thread
	 * should block. */
	if(pthread_mutex_lock(&pthread_cancel_1_3_mutex) != 0)
        {
	 ...
	 * pthread_testcancel(). */
	pthread_cancel_1_3_cleanup_flag=-2;
	pthread_exit(0);
	return NULL;
}

int pthread_cancel_1_3()
{
	pthread_t new_th;

	/* Initializing values */
	pthread_cancel_1_3_sem1=INTHREAD;
	pthread_cancel_1_3_cleanup_flag=0;
	
	/* Lock the pthread_cancel_1_3_mutex */
	if(pthread_mutex_lock(&pthread_cancel_1_3_mutex) != 0)
	{
		perror("Error in pthread_mutex_lock()\n");
	...
	/* Create a new thread. */
	if(pthread_create(&new_th, NULL, pthread_cancel_1_3_a_thread_func, NULL) != 0)
	{	
	 ...
	 * pthread_cancel_1_3_a_thread_func() to set pthread_cancel_1_3_sem1=INMAIN.) */
	while(pthread_cancel_1_3_sem1==INTHREAD)
		sleep(1);
	...
	/* Cancel request has been sent, unlock the pthread_cancel_1_3_mutex */
	if(pthread_mutex_unlock(&pthread_cancel_1_3_mutex) != 0)
	{
	...
	if(pthread_cancel_1_3_cleanup_flag == -1)
	{
		printf("Test FAILED: Cancel request was not deferred.\n");
		return PTS_FAIL;
	}	
	
	if(pthread_cancel_1_3_cleanup_flag == -2)
	{
