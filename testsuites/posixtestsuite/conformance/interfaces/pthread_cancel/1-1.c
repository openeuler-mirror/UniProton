...
int pthread_cancel_1_1_sem1;		/* Manual semaphore */
int pthread_cancel_1_1_cleanup_flag;

/* Cleanup function that the thread executes when it is canceled.  So if
 * pthread_cancel_1_1_cleanup_flag is -1, it means that the thread was canceled. */
void pthread_cancel_1_1_a_cleanup_func(void *unused)
{
	pthread_cancel_1_1_cleanup_flag=1;
	return;
}

/* Function that the thread executes upon its creation */
void *pthread_cancel_1_1_a_thread_func()
{
	pthread_setcancelstate(PTHREAD_CANCEL_ENABLE, NULL);
	pthread_setcanceltype(PTHREAD_CANCEL_ASYNCHRONOUS, NULL);
	
	pthread_cleanup_push(pthread_cancel_1_1_a_cleanup_func,NULL);
	
	/* Indicate to main() that the thread has been created. */	
	pthread_cancel_1_1_sem1=INMAIN;
	
	/* Wait until main() has sent out a cancel request, meaning until it
	 * sets pthread_cancel_1_1_sem1==0 */ 
	while(pthread_cancel_1_1_sem1==1)
		sleep(1);
...
int pthread_cancel_1_1()
{
	pthread_t new_th;

	/* Initializing values */
	pthread_cancel_1_1_sem1=INTHREAD;
	pthread_cancel_1_1_cleanup_flag=0;
	
	/* Create a new thread. */
	if(pthread_create(&new_th, NULL, pthread_cancel_1_1_a_thread_func, NULL) != 0)
	{	
	 ...
	 * pthread_cancel_1_1_a_thread_func() to set pthread_cancel_1_1_sem1=1.) */
	while(pthread_cancel_1_1_sem1==INTHREAD)
		sleep(1);
	...
	while(pthread_cancel_1_1_cleanup_flag==INTHREAD)
		sleep(1);
	...
	if(pthread_cancel_1_1_cleanup_flag < 0)
	{
