...
int pthread_cancel_4_1_sem; 		/* Manual semaphore */
void *pthread_cancel_4_1_a_thread_func()
{
	pthread_setcanceltype(PTHREAD_CANCEL_ASYNCHRONOUS, NULL);
	
	/* Indicate to main() that the thread has been created. */	
	pthread_cancel_4_1_sem=1;
...
int pthread_cancel_4_1()
{
	pthread_t new_th;
	int ret; 
	
	pthread_cancel_4_1_sem=0;	
	
	/* Create a new thread. */
	if(pthread_create(&new_th, NULL, pthread_cancel_4_1_a_thread_func, NULL) != 0)
	{	
	...
	while(pthread_cancel_4_1_sem==0)
		sleep(1);
