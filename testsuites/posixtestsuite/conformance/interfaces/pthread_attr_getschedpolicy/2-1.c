...
// todo: pthread_attr_getschedpolicy_2_1 仅支持PRT_SCHED_FIFO 用例其他优先级替换方便以后
#define RRPOLICY SCHED_FIFO
#define OTHERPOLICY SCHED_FIFO

int pthread_attr_getschedpolicy_2_1_verify_policy(pthread_attr_t *attr, int policytype) {
	int rc;
	int policy;
...
int pthread_attr_getschedpolicy_2_1()
{
	int                   rc=0;
	pthread_attr_t        attr;
	...
	if( rc != 0) {
  	...
  	pthread_attr_getschedpolicy_2_1_verify_policy(&attr, FIFOPOLICY);
	...
	if( rc != 0) {
  	...
  	pthread_attr_getschedpolicy_2_1_verify_policy(&attr, RRPOLICY);
	...
	if( rc != 0) {
  	...
  	pthread_attr_getschedpolicy_2_1_verify_policy(&attr, OTHERPOLICY);
