 ...
 * Test that pthread_rwlock_rdlock(pthread_rwlock_t *pthread_rwlock_rdlock_2_2_rwlock)
 *
 ...
 * 2.  Main thread read lock 'pthread_rwlock_rdlock_2_2_rwlock'
 * 3.  Create a writer thread, with schedule policy as "SCHED_FIFO", and priority
 * 	using sched_get_priority_min()+1.
 * 4.  The thread write lock 'pthread_rwlock_rdlock_2_2_rwlock', should block.
 * 5.  Main thread create a reader thread, with schedule policy as "SCHED_FIFO", and
 *     priority sched_get_priority_min()+1
 * 6.  Reader thread read lock 'pthread_rwlock_rdlock_2_2_rwlock', should block, since there is an equal  priority
 *     writer blocked on 'pthread_rwlock_rdlock_2_2_rwlock'
 * 7.  Main thread release the 'pthread_rwlock_rdlock_2_2_rwlock', the writer should get the lock first
 */
...
static pthread_rwlock_t pthread_rwlock_rdlock_2_2_rwlock;
static int pthread_rwlock_rdlock_2_2_rd_thread_state;
static int pthread_rwlock_rdlock_2_2_wr_thread_state; 
...
static int pthread_rwlock_rdlock_2_2_set_priority(pthread_t pid, unsigned policy, unsigned prio)
{
...
static void* pthread_rwlock_rdlock_2_2_fn_rd(void *arg)
{ 
	int rc = 0;
	int priority;
	pthread_rwlock_rdlock_2_2_rd_thread_state = ENTERED_THREAD;
	
	priority = (long)arg;
	pthread_rwlock_rdlock_2_2_set_priority(pthread_self(), TRD_POLICY, priority);

	printf("rd_thread: attempt read lock\n");
	rc = pthread_rwlock_rdlock(&pthread_rwlock_rdlock_2_2_rwlock);
	if(rc != 0)
	{
	...
	if(pthread_rwlock_unlock(&pthread_rwlock_rdlock_2_2_rwlock) != 0)
	{
		printf("rd_thread: Error at pthread_rwlock_unlock()\n");
		return PTS_UNRESOLVED ;
	}
	pthread_rwlock_rdlock_2_2_rd_thread_state = EXITING_THREAD;
	pthread_exit(0);
	return NULL;
}
 
static void* pthread_rwlock_rdlock_2_2_fn_wr(void *arg)
{ 
	int rc = 0;
	int priority;
	pthread_rwlock_rdlock_2_2_wr_thread_state = ENTERED_THREAD;
	
	priority = (long)arg;
	pthread_rwlock_rdlock_2_2_set_priority(pthread_self(), TRD_POLICY, priority);

	printf("wr_thread: attempt write lock\n");
	rc = pthread_rwlock_wrlock(&pthread_rwlock_rdlock_2_2_rwlock);
	if(rc != 0)
	{
	...
	pthread_rwlock_rdlock_2_2_wr_thread_state = EXITING_THREAD;
	pthread_exit(0);
	return NULL;
}
 
int pthread_rwlock_rdlock_2_2()
{
	...
	if(pthread_rwlock_init(&pthread_rwlock_rdlock_2_2_rwlock, NULL) != 0)
	{
	...
	if(pthread_rwlock_rdlock(&pthread_rwlock_rdlock_2_2_rwlock) != 0)
	{
	...
	pthread_rwlock_rdlock_2_2_wr_thread_state = NOT_CREATED_THREAD;
	priority = sched_get_priority_min(TRD_POLICY) + 1;
	printf("main: create wr_thread, with priority: %d\n", priority);
	if(pthread_create(&wr_thread, NULL, pthread_rwlock_rdlock_2_2_fn_wr, (void*)(long)priority) != 0)
	{
	...
	do{
		sleep(1);
	}while (pthread_rwlock_rdlock_2_2_wr_thread_state !=EXITING_THREAD && cnt++ < 3); 
	
	if(pthread_rwlock_rdlock_2_2_wr_thread_state == EXITING_THREAD )
	{
		printf("wr_thread did not block on write lock, when a reader owns the lock\n");
		return PTS_UNRESOLVED ;
	}
	else if(pthread_rwlock_rdlock_2_2_wr_thread_state != ENTERED_THREAD)
	{
		printf("Unexpected wr_thread state: %d\n", pthread_rwlock_rdlock_2_2_wr_thread_state);
		return PTS_UNRESOLVED ;
	}
	
	pthread_rwlock_rdlock_2_2_rd_thread_state = NOT_CREATED_THREAD;
	priority = sched_get_priority_min(TRD_POLICY) + 1;
	printf("main: create rd_thread, with priority: %d\n", priority);
	if(pthread_create(&rd_thread, NULL, pthread_rwlock_rdlock_2_2_fn_rd, (void*)(long)priority) != 0)
	{
	...
	do{
		sleep(1);
	}while (pthread_rwlock_rdlock_2_2_rd_thread_state !=EXITING_THREAD && cnt++ < 3); 
	
	if(pthread_rwlock_rdlock_2_2_rd_thread_state == EXITING_THREAD )
	{
	...
	else if(pthread_rwlock_rdlock_2_2_rd_thread_state != ENTERED_THREAD)
	{
		printf("Unexpected rd_thread state: %d\n", pthread_rwlock_rdlock_2_2_rd_thread_state);
		return PTS_UNRESOLVED ;
	}
	...
	if(pthread_rwlock_rdlock_2_2_wr_thread_state == ENTERED_THREAD )
	{
		printf("Test FAILED: higher priority wr_thread still blocked on write lock, when a reader released the lock\n");
		return PTS_FAIL ;
	}
	else if(pthread_rwlock_rdlock_2_2_wr_thread_state != EXITING_THREAD)
	{
		printf("Unexpected wr_thread state: %d\n", pthread_rwlock_rdlock_2_2_wr_thread_state);
		return PTS_UNRESOLVED ;
	}
	...
	if(pthread_rwlock_rdlock_2_2_rd_thread_state == ENTERED_THREAD )
	{
		printf("Test FAILED: rd_thread still blocked on read lock when the lock has no owner\n");
		return PTS_FAIL ;
	}
	else if(pthread_rwlock_rdlock_2_2_rd_thread_state != EXITING_THREAD)
	{
		printf("Unexpected rd_thread state: %d\n", pthread_rwlock_rdlock_2_2_rd_thread_state);
		return PTS_UNRESOLVED ;
	}
	...
	if(pthread_rwlock_destroy(&pthread_rwlock_rdlock_2_2_rwlock) != 0)
	{
