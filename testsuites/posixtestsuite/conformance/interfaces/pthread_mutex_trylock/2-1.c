...
} pthread_mutex_trylock_2_1_testdata_t;

struct _pthread_mutex_trylock_2_1_scenar
{
...
pthread_mutex_trylock_2_1_scenarii[] =
{
...
#define NSCENAR (sizeof(pthread_mutex_trylock_2_1_scenarii)/sizeof(pthread_mutex_trylock_2_1_scenarii[0]))

/* The test function will only perform a trylock operation then return. */
void * pthread_mutex_trylock_2_1_tf(void * arg)
{
	pthread_mutex_trylock_2_1_testdata_t * td = (pthread_mutex_trylock_2_1_testdata_t *)arg;
...
/* Main entry point. */
int pthread_mutex_trylock_2_1(int argc, char * argv[])
{
	int ret;
	int sc;
	pthread_mutexattr_t ma;
	
	pthread_mutex_trylock_2_1_testdata_t * td;
	pthread_mutex_trylock_2_1_testdata_t alternativ;
 ...
 */
	...
	else
	{
		...
		td = (pthread_mutex_trylock_2_1_testdata_t *) mmaped;
 ...
 */
	for ( sc=0; sc < NSCENAR ; sc++)
	{
		...
		/* Set the mutex type */
		ret = pthread_mutexattr_settype(&ma, pthread_mutex_trylock_2_1_scenarii[sc].m_type);
		if (ret != 0)  {  UNRESOLVED(ret, "[parent] Unable to set mutex type");  }
		#if VERBOSE > 1
		pthread_mutex_trylock_output("[parent] Mutex type : %i\n", pthread_mutex_trylock_2_1_scenarii[sc].m_type);
		#endif
			
		/* Set the pshared attributes, if supported */
		if ((pshared > 0) && (pthread_mutex_trylock_2_1_scenarii[sc].m_pshared != 0))
		{
		...
		if ((pshared > 0) && (pthread_mutex_trylock_2_1_scenarii[sc].fork != 0))
		{
			do_fork = 1;
			#if VERBOSE > 1
...
/**********
 * Initialize the pthread_mutex_trylock_2_1_testdata_t structure with the previously defined attributes 
 */
 ...
 */
		...
		if (pthread_mutex_trylock_2_1_scenarii[sc].m_type == PTHREAD_MUTEX_RECURSIVE)
		{
		...
		if (do_fork != 0)
		{
			...
			if (child_pr == 0)
			{
				...
				if (pthread_mutex_trylock_2_1_tf((void *)td) != NULL)
				{
		...
		else /* do_fork == 0 */
		{
			...
			ret = pthread_create(&child_th, NULL, pthread_mutex_trylock_2_1_tf, td);
