...
#include <stdlib.h>
#include "posixtest.h"
#define CLOCK_SETTIME_GET_BEFORE_TIME clock_settime_4_3_getBeforeTime
#define CLOCK_SETTIME_SET_BEFORE_BACK clock_settime_4_3_setBackTime
#include "../helpers.h"
...
int clock_settime_4_3_caught = 0;

void clock_settime_4_3_handler(int signo)
{
	printf("Caught signal\n");
	clock_settime_4_3_caught++;
}

int clock_settime_4_3(int argc, char *argv[])
{
	struct sigevent ev;
	...
	act.sa_handler=clock_settime_4_3_handler;
	act.sa_flags=0;
	...
	CLOCK_SETTIME_GET_BEFORE_TIME(&tpreset);
	...
	CLOCK_SETTIME_SET_BEFORE_BACK(tpreset);

	if (clock_settime_4_3_caught == 1) {
