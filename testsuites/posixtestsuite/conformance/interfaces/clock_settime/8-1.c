...
#include <sys/wait.h>
#include "posixtest.h"
#define CLOCK_SETTIME_GET_BEFORE_TIME clock_settime_8_1_getBeforeTime
#define CLOCK_SETTIME_SET_BEFORE_BACK clock_settime_8_1_setBackTime
#include "helpers.h"
...
int clock_settime_8_1(int argc, char *argv[])
{
	...
	if ((pid = fork()) == 0) {
		...
		CLOCK_SETTIME_GET_BEFORE_TIME(&tsreset); // get current time
