...
#include <sys/wait.h>
#include "posixtest.h"
#define CLOCK_SETTIME_GET_BEFORE_TIME clock_settime_7_2_getBeforeTime
#define CLOCK_SETTIME_SET_BEFORE_BACK clock_settime_7_2_setBackTime
#include "helpers.h"
...
int clock_settime_7_2(int argc, char *argv[])
{
	...
	if ((pid = fork()) == 0) {
		...
		CLOCK_SETTIME_SET_BEFORE_BACK(tsreset); //should be ~= before time
