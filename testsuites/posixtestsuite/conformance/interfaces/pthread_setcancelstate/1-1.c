 ...
 *    PTHREAD_CANCEL_ENABLE and the pthread_setcancelstate_1_1_cancel_flag to 1.
 * 3. Send out a thread cancel request to the new thread
 * 4. If the cancel request was honored, the pthread_setcancelstate_1_1_cancel_flag will remain 1.
 * 5. If not, the thread will continue until the end of execution, pthread_setcancelstate_1_1_cancel_flag will be set to -1
...
int pthread_setcancelstate_1_1_sem1;		/* Manual semaphore */
int pthread_setcancelstate_1_1_cancel_flag;

/* Function that the thread executes upon its creation */
void *pthread_setcancelstate_1_1_a_thread_func()
{
	...
	pthread_setcancelstate_1_1_cancel_flag=1;

	/* Indicate to main() that the thread has been created. */	
	pthread_setcancelstate_1_1_sem1=INMAIN;

	/* Wait until main() has sent out a cancel request, meaning until it
	 * sets pthread_setcancelstate_1_1_sem1==INTHREAD. */
	while(pthread_setcancelstate_1_1_sem1==INMAIN)
		sleep(1);
	
	/* If the thread correctly honors the cancel request, then the pthread_setcancelstate_1_1_cancel_flag will
	 * remain 1.  If it contiues on with the thread execution, then the pthread_setcancelstate_1_1_cancel_flag
...
int pthread_setcancelstate_1_1()
{
	pthread_t new_th;
		
	/* Initializing values */
	pthread_setcancelstate_1_1_sem1=INTHREAD;
	pthread_setcancelstate_1_1_cancel_flag=0;
	
	/* Create a new thread. */
	if(pthread_create(&new_th, NULL, pthread_setcancelstate_1_1_a_thread_func, NULL) != 0)
	{	
	 ...
	 * pthread_setcancelstate_1_1_a_thread_func() to set pthread_setcancelstate_1_1_sem1=INMAIN.) */
	while(pthread_setcancelstate_1_1_sem1==INTHREAD)
		sleep(1);
	 ...
	 * the test fails. */
	if(pthread_setcancelstate_1_1_cancel_flag < 0)
	{
