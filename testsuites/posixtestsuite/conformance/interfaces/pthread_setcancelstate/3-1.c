...
int pthread_setcancelstate_3_1_ret; 		/* Return value of pthread_setcancelstate(). */

/* Function that the thread executes upon its creation */
void *pthread_setcancelstate_3_1_a_thread_func()
{
	/* Set cancel state to an invalid integer and save the return value. */
	pthread_setcancelstate_3_1_ret=pthread_setcancelstate(-100, NULL);

	pthread_exit(0);
	return NULL;
}

int pthread_setcancelstate_3_1()
{
	pthread_t new_th;
		
	/* Initializing value */
	pthread_setcancelstate_3_1_ret=0;
	
	/* Create a new thread. */
	if(pthread_create(&new_th, NULL, pthread_setcancelstate_3_1_a_thread_func, NULL) != 0)
	{	
