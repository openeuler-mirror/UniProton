...
int pthread_join_3_1_cleanup_flag;	/* Flag to indicate the thread's cleanup handler was called */

/* Cleanup function that the thread executes when it is canceled.  So if
 * pthread_join_3_1_cleanup_flag is 1, it means that the thread was canceled. */
void pthread_join_3_1_a_cleanup_func(void *unused)
{
	pthread_join_3_1_cleanup_flag=1;
	return;
}

/* Thread's function. */
void *pthread_join_3_1_a_thread_func()
{
	pthread_setcancelstate(PTHREAD_CANCEL_ENABLE, NULL);
	pthread_setcanceltype(PTHREAD_CANCEL_DEFERRED, NULL);
	
	/* Set up the cleanup handler */
	pthread_cleanup_push(pthread_join_3_1_a_cleanup_func,NULL);

	// 增加调度点
	sched_yield();

	// 增加testcancel, 因设置延时取消, 所以需要加testcancel
	pthread_testcancel();
...
int pthread_join_3_1()
{
	pthread_t new_th;

	/* Initialize flag */
	pthread_join_3_1_cleanup_flag = 0;
	
	/* Create a new thread. */
	if(pthread_create(&new_th, NULL, pthread_join_3_1_a_thread_func, NULL) != 0)
	{	
