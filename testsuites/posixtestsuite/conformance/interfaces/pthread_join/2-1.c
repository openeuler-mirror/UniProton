...
int pthread_join_2_1_sem;	/* Manual semaphore used to indicate when the thread has been created. */

/* Thread's function. */
void *pthread_join_2_1_a_thread_func()
{
	pthread_join_2_1_sem=INMAIN;
...
int pthread_join_2_1()
{
	...
	value_ptr=0;
	pthread_join_2_1_sem=INTHREAD;
	
	/* Create a new thread. */
	if(pthread_create(&new_th, NULL, pthread_join_2_1_a_thread_func, NULL) != 0)
	{	
	...
	while(pthread_join_2_1_sem==INTHREAD)
		sleep(1);
