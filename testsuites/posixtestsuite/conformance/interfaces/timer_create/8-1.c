 ...
 * Steps:
 * 1.  Set up a signal timer_create_8_1_handler for timer in parent.
 ...
 * 5.  Set up a signal timer_create_8_1_handler for the timer in the parent and ensure it
 *     is not called by ensuring the child is able to sleep uninterrupted. 
 *     [Note:  The delay to set up this timer_create_8_1_handler could also cause false
 *     results.]
 *
...
void timer_create_8_1_parenthandler(int signo)
{
	printf("Expected - Caught signal\n");
}

void timer_create_8_1_handler(int signo)
{
	printf("Not expected - Caught signal\n");
}

int timer_create_8_1(int argc, char *argv[])
{
	timer_t tid;
	...
	actp.sa_handler=timer_create_8_1_parenthandler;
	actp.sa_flags=0;
	...
	if ((pid = fork()) == 0) {
		...
		act.sa_handler=timer_create_8_1_handler;
		act.sa_flags=0;
