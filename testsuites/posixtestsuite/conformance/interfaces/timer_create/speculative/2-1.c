...
int timer_create_2_1_compare(const void *key, const void *amemb)
{
...
int timer_create_2_1(int argc, char *argv[])
{
	struct sigevent ev;
	timer_t tid;
	timer_t *tids;
	...
	for (i=0; i<max;i++) {
		...
		tids[i] = tid;
		if (lfind(&tid, tids, &i, sizeof(timer_t), timer_create_2_1_compare) != NULL) {
