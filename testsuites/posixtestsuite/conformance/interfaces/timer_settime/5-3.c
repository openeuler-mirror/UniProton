...
int timer_settime_5_3_fails = 0, timer_settime_5_3_passes = 0;

void handler(int signo)
{
	printf("Caught signal\n");
	timer_settime_5_3_passes += 1;
}

int timer_settime_5_3(int argc, char *argv[])
{
	struct sigevent ev;
	...
	timer_settime_5_3_fails = NUMTESTS-timer_settime_5_3_passes;

	printf("timer_settime_5_3_passes %d, timer_settime_5_3_fails %d\n", timer_settime_5_3_passes, timer_settime_5_3_fails);

	if (timer_settime_5_3_fails > 0) {
		return PTS_FAIL;
	} else {
		return PTS_PASS;
	}
