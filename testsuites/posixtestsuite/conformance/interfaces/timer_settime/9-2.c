...
static int timer_settime_9_2_timeroffsets[NUMTESTS][2] = { {0, 90000000}, {1, 0}, 
					{1, 30000000}, {2, 0},
					{3, 5000}, {4, 5} };

int timer_settime_9_2(int argc, char *argv[])
{
	struct sigevent ev;
	timer_t tid;
	...
	for (i = 0; i < NUMTESTS; i++) {
		...
		if (tsbefore.tv_nsec + timer_settime_9_2_timeroffsets[i][1] < 1000000000) {
			its.it_value.tv_sec = tsbefore.tv_sec + 
						timer_settime_9_2_timeroffsets[i][0];
			its.it_value.tv_nsec = tsbefore.tv_nsec + 
						timer_settime_9_2_timeroffsets[i][1];
		} else {
			its.it_value.tv_sec = tsbefore.tv_sec + 
						timer_settime_9_2_timeroffsets[i][0] + 1;
			its.it_value.tv_nsec = tsbefore.tv_nsec + 
						timer_settime_9_2_timeroffsets[i][1] -
						1000000000;
		}
