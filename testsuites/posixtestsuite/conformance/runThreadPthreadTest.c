...
#include "prt_mem.h"
#include "prt_sem.h"
#include "runThreadPthreadTest.h"
...
void Init(uintptr_t param1, uintptr_t param2, uintptr_t param3, uintptr_t param4)
{
    ...
    for (i = 0; i < sizeof(run_test_arry_2)/sizeof(test_run_main *); i++) {
        run = run_test_arry_2[i];
        printf("Runing %s test...\n", run_test_name_2[i]);
        ret = run();
        if (ret != 0) {
            failCount++;
    ...
    for (i = 0; i < sizeof(run_test_arry_3)/sizeof(test_run_main *); i++) {
        run = run_test_arry_3[i];
        printf("Runing %s test...\n", run_test_name_3[i]);
        ret = run();
        if (ret != 0) {
            failCount++;
