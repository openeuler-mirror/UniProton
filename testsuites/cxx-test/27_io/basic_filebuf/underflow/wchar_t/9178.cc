...
class checksumcvt9178 : public std::codecvt<InternT, char, StateT>
{
...
public:
  ...
  explicit checksumcvt9178(size_t refs = 0)
  : Base(refs)
  { }
...
static void test01()
{
  ...
  locale loc;
  loc = locale(loc, new checksumcvt9178<wchar_t>);

  wfilebuf fbuf1;
  fbuf1.pubimbue(loc);
  fbuf1.open("/tmp/9178", ios_base::out | ios_base::trunc);
  ...
  wfilebuf fbuf2;
  fbuf2.pubimbue(loc);
  fbuf2.open("/tmp/9178", std::ios_base::in);
...
int test_27_io_basic_filebuf_underflow_wchar_t_9178()
{
  test01();
  return 0;
}
