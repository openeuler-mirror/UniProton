...
struct CloseCvt : std::codecvt<wchar_t, char, std::mbstate_t>
{
  explicit CloseCvt(size_t refs) : codecvt(refs) { }
...
test01()
{
  ...
  {
    filebuf fb(&conv);
    fb.open("/tmp/basic_filebuf_close_81256", std::wios::out);
    fb.sputn(L"x", 1);
...
int
test_27_io_basic_filebuf_close_81256()
{
  test01();
  test02();
  return 0;
