...
if $sim_flag; then
    cmake -S .. -B $TMP_DIR -DAPP:STRING=$APP -DTOOLCHAIN_PATH:STRING=$TOOLCHAIN_PATH -DSIM:STRING="_SIM_" -DLIB_RUN_TYPE:STRING=$LIB_RUN_TYPE -DCPU_TYPE:SRTING="m4" -DCMAKE_TRY_COMPILE_TARGET_TYPE=STATIC_LIBRARY
else
    cmake -S .. -B $TMP_DIR -DAPP:STRING=$APP -DTOOLCHAIN_PATH:STRING=$TOOLCHAIN_PATH -DLIB_RUN_TYPE:STRING=$LIB_RUN_TYPE -DCPU_TYPE:SRTING="m4" -DCMAKE_TRY_COMPILE_TARGET_TYPE=STATIC_LIBRARY
fi
pushd $TMP_DIR
make $APP --trace
popd
